...
#define TOGGLE_INVERT (1 << 3) // UI removal checkbox; stored bool retains its original meaning.
#define DROPDOWN_AUTOUPDATE (1 << 3)

NAMESPACE_BEGIN(Vars)
	NAMESPACE_BEGIN(Menu)
		CVar(CheatTitle, "Cheat title", std::string("Nikogram"), VISUAL | DROPDOWN_AUTOUPDATE);
		CVar(CheatTag, "Cheat tag", std::string("[Nikogram]"), VISUAL);
	...
	NAMESPACE_BEGIN(Aimbot)
		...
		NAMESPACE_BEGIN(Hitscan)
			...
			CVar(Extrapolation, "Extrapolation", false);
			CVar(ExtrapolationGuide, "Show extrapolation", false, VISUAL);
			CVar(ExtrapolationRange, "Extrapolate range", 50, SLIDER_MIN, 0, 100, 5, "%d ms");
	...
	NAMESPACE_BEGIN(Backtrack)
		CVar(RealLag, "Real lag", 0, SLIDER_CLAMP, 0, 1000, 5, "%d ms");
	...
	NAMESPACE_BEGIN(Visuals)
		...
		NAMESPACE_BEGIN(Viewmodel)
			...
			CVar(CrosshairCooldown, "Predict during Cooldown##Crosshair", false, VISUAL);
			CVar(ViewmodelCooldown, "Predict during Cooldown##Viewmodel", false, VISUAL);
		...
		NAMESPACE_BEGIN(Hitbox)
			...
			CVarEnum(BoundsEnabled, VA_LIST("Bounds enabled", "Hitbox bounds enabled"), 0b000, VISUAL | DROPDOWN_MULTI, "Off",
				VA_LIST("On shot", "On hit", "Aim point", "Predict during cooldown"),
				OnShot = 1 << 0, OnHit = 1 << 1, AimPoint = 1 << 2, PredictDuringCooldown = 1 << 3);
