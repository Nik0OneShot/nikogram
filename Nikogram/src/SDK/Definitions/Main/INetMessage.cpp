...
// ToString results must outlive the call, so keep them in a small ring of buffers
// (no thread_local: implicit TLS isn't set up when the dll is manually mapped)
static std::mutex s_mNetMsgString;
static std::string s_aNetMsgStrings[8];
static size_t s_iNetMsgString = 0;
static const char* StoreNetMsgString(std::string sString)
{
    std::lock_guard tLock(s_mNetMsgString);
    std::string& sBuffer = s_aNetMsgStrings[s_iNetMsgString++ % std::size(s_aNetMsgStrings)];
    sBuffer = std::move(sString);
...
{
    if (!m_ConVars.Count())
        return StoreNetMsgString(std::format("{}: 0 cvars", GetName()));
    return StoreNetMsgString(std::format("{}: {} cvars, \"{}\"=\"{}\"", GetName(), m_ConVars.Count(), m_ConVars[0].Name, m_ConVars[0].Value));
}
...
{
    return StoreNetMsgString(std::format("net_SignonState: state {}, count {}", m_nSignonState, m_nSpawnCount));
}
...
{
    return StoreNetMsgString(std::format("{}: tick {}", GetName(), m_nTick));
}

const char* CLC_VoiceData::ToString(void) const
{
    return StoreNetMsgString(std::format("{}: {} bytes", GetName(), static_cast<int>(m_nLength * 0.125f)));
}
...
{
    return StoreNetMsgString(std::format("{}: tick {}", GetName(), m_nBaselineTick));
}