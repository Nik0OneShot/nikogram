...
// value strings are shared with the engine, which allocates and frees them through tier0
static void* KVAlloc(size_t nSize)
{
	return I::MemAlloc->Alloc(nSize);
}

static void KVFree(void* pMem)
{
	if (pMem)
		I::MemAlloc->Free(pMem);
}
...
void KeyValues::SetWString(const char* keyName, const wchar_t* value)
{
	...
	if (dat)
	{
		...
		dat->m_wsValue = reinterpret_cast<wchar_t*>(KVAlloc((len + 1) * sizeof(wchar_t)));
...
void KeyValues::SetString(const char* keyName, const char* value)
{
	...
	if (dat)
	{
		...
		dat->m_sValue = reinterpret_cast<char*>(KVAlloc(len + 1));
...
void KeyValues::SetUint64(const char* keyName, uint64_t value)
{
	...
	if (dat)
	{
		...
		dat->m_sValue = reinterpret_cast<char*>(KVAlloc(sizeof(uint64_t)));
