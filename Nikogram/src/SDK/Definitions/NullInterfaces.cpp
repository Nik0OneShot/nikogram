...
bool CNullInterfaces::Initialize()
{
	...
	auto pKeyValuesSystem = U::Memory.GetModuleExport<IKeyValuesSystem*(*)()>("vstdlib.dll", "KeyValuesSystem");
	I::KeyValuesSystem = pKeyValuesSystem ? pKeyValuesSystem() : nullptr;
	Validate(I::KeyValuesSystem);

	const HSteamPipe hsNewPipe = m_hSteamPipe = I::SteamClient->CreateSteamPipe();
	Validate(hsNewPipe);

	const HSteamPipe hsNewUser = m_hSteamUser = I::SteamClient->ConnectToGlobalUser(hsNewPipe);
	Validate(hsNewUser);
...
void CNullInterfaces::Unload()
{
	// the interfaces obtained through this pipe are invalid afterwards
	if (!I::SteamClient || !m_hSteamPipe)
		return;

	if (m_hSteamUser)
		I::SteamClient->ReleaseUser(m_hSteamPipe, m_hSteamUser);
	I::SteamClient->BReleaseSteamPipe(m_hSteamPipe);
	m_hSteamPipe = m_hSteamUser = 0;
}