...
static inline void PrintChat(const std::string& sText)
{
	// ChatPrintf treats its argument as a printf format string, so escape '%' (player names can contain it)
	if (!I::ClientModeShared || !I::ClientModeShared->m_pChatElement)
		return;

	std::string sEscaped;
	sEscaped.reserve(sText.size());
	for (char c : sText)
	{
		sEscaped += c;
		if (c == '%')
	...
	I::ClientModeShared->m_pChatElement->ChatPrintf(0, sEscaped.c_str());
}
	...
	const char* sLeft, const char* sRight)
{
	if (sLog)
	{
		...
		if (iTo & OUTPUT_CHAT)
			PrintChat(std::format("{}{}{}{}\x1 {}", tColor.ToHex(), sLeft, sFunction, sRight, sLog));
		if (iTo & OUTPUT_PARTY)
	...
	else
	{
		...
		if (iTo & OUTPUT_CHAT)
			PrintChat(std::format("{}{}\x1", tColor.ToHex(), sFunction));
		if (iTo & OUTPUT_PARTY)
...
void SDK::SetClipboard(const std::string& sString)
{
	...
	std::wstring sWide = ConvertUtf8ToWide(sString);
	const size_t nBytes = (sWide.length() + 1) * sizeof(wchar_t);
	if (HGLOBAL hMemory = GlobalAlloc(GMEM_MOVEABLE, nBytes))
	{
		bool bSet = false;
		if (void* pData = GlobalLock(hMemory))
		{
			memcpy(pData, sWide.c_str(), nBytes);
			GlobalUnlock(hMemory);
			bSet = SetClipboardData(CF_UNICODETEXT, hMemory) != nullptr;
		}
		if (!bSet)
			GlobalFree(hMemory); // the clipboard only takes ownership on success
	}
...
std::string SDK::GetClipboard()
{
	...
	{
		if (auto pData = static_cast<const wchar_t*>(GlobalLock(hData)))
		{
			sString = ConvertWideToUTF8(pData);
			GlobalUnlock(hData);
		}
	}
	else if (HANDLE hText = GetClipboardData(CF_TEXT))
	{
		if (auto pData = static_cast<const char*>(GlobalLock(hText)))
		{
			sString = pData;
			GlobalUnlock(hText);
		}
...
Vec3 SDK::ComputeMove(const CUserCmd* pCmd, CTFPlayer* pLocal, const Vec3& vFrom, const Vec3& vTo)
{
	...
	const float flPitch = Math::Deg2Rad(Math::VectorAngles(vDiff).x); // vAngle is from the 2D vector so its pitch is always 0

	Vec3 vMove = { cos(flYaw) * 450.f, -sin(flYaw) * 450.f, -sin(flPitch) * 450.f }; // positive (swim up) when the target is above
