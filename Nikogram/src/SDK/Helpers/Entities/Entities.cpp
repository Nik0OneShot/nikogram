...
void CEntities::Store()
{
	...
	for (auto pEntity : H::Entities.GetGroup(EntityEnum::PlayerAll))
	{
		...
		for (int i = 0; i < iDeltaTicks; i++)
		{
			...
			F::AnimInterp.Record(pPlayer);
		}
		I::GlobalVars->frametime = flOldFrameTime;
		F::AnimInterp.EndBurst(pPlayer);
	}
}
...
void CEntities::ManualNetwork(const StartSoundParams_t& params)
{
	...
	case ETFClassID::CTFPlayer:
		if (float flDelta = std::min(I::GlobalVars->curtime - s_mDormancy[n].m_flLastUpdate, 1.f); flDelta > 0.f) // multiple sounds in one tick
		{
			pEntity->As<CTFPlayer>()->m_vecVelocity() = (params.origin - pEntity->m_vecOrigin()) / flDelta;
...
Vec3 CEntities::GetDeltaAngles(uint16_t iIndex) { return iIndex < MAX_PLAYERS && m_aLagTimes[iIndex] ? m_aEyeAngles[iIndex].DeltaAngle(m_aOldAngles[iIndex]) / m_aLagTimes[iIndex] * (F::Backtrack.GetReal() + TICKS_TO_TIME(F::Backtrack.GetAnticipatedChoke())) : Vec3(); }
...
// read without inserting default entries
template <class K, class V>
static V MapGet(const std::unordered_map<K, V>& mMap, const K& tKey, V tDefault)
{
	auto it = mMap.find(tKey);
	return it != mMap.end() ? it->second : tDefault;
}

int CEntities::GetPriority(int iIndex) { return MapGet(m_mIPriorities, iIndex, 0); }
int CEntities::GetPriority(uint32_t uAccountID) { return MapGet(m_mUPriorities, uAccountID, 0); }
bool CEntities::IsFriend(int iIndex) { return MapGet(m_mIFriends, iIndex, false); }
bool CEntities::IsFriend(uint32_t uAccountID) { return MapGet(m_mUFriends, uAccountID, false); }
bool CEntities::InParty(int iIndex) { return iIndex != I::EngineClient->GetLocalPlayer() && MapGet(m_mIParty, iIndex, 0) == 1; }
bool CEntities::InParty(uint32_t uAccountID) { return uAccountID != m_uAccountID && MapGet(m_mUParty, uAccountID, 0) == 1; }
bool CEntities::IsF2P(int iIndex) { return MapGet(m_mIF2P, iIndex, false); }
bool CEntities::IsF2P(uint32_t uAccountID) { return MapGet(m_mUF2P, uAccountID, false); }
int CEntities::GetLevel(int iIndex) { return MapGet(m_mILevels, iIndex, -2); }
int CEntities::GetLevel(uint32_t uAccountID) { return MapGet(m_mULevels, uAccountID, -2); }
int CEntities::GetParty(int iIndex) { return MapGet(m_mIParty, iIndex, 0); }
int CEntities::GetParty(uint32_t uAccountID) { return MapGet(m_mUParty, uAccountID, 0); }
