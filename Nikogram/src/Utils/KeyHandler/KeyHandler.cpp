...
static inline bool VirtualKeyUnicode(byte iKey, std::string& sKey, HKL pKeyboard = nullptr)
{
	...
	wchar_t buffer[8] = {};
	int iChars = ToUnicodeEx(iKey, uScanCode, aKeyState, buffer, 7, 4, pKeyboard); // leave room for the terminator, ToUnicodeEx doesn't write one
	if (iChars > 0 && F::Render.FontRegular->IsGlyphInFont(buffer[0]))
	{
		buffer[iChars] = L'\0';
