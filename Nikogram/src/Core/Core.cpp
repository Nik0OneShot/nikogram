...
static inline bool CheckDXLevel()
{
	...
	{
		/*
		const char* sMessage = "You are running with graphics options that Nikogram does not support. -dxlevel must be at least 90.";
		U::Core.AppendFailText(sMessage);
		SDK::Output("Nikogram", sMessage, ERROR_COLOR, OUTPUT_CONSOLE | OUTPUT_MENU | OUTPUT_DEBUG);
		return false;
		*/

		const char* sMessage = "You are running with graphics options that Nikogram does not support. It is recommended for -dxlevel to be at least 90.";
		SDK::Output("Nikogram", sMessage, WARNING_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG, ICON_MD_WARNING);
	}
...
void CCore::LogFailText()
{
	try
	{
		...
		m_ssFailStream << "Logged to Nikogram\\fail_log.txt. ";
	}
	catch (...) {}
