	...
	SVC_PacketEntities* entmsg)
{
	...
	{
		SDK::Output("ProcessPacketEntities", "Failed to restore weapon crit data! (1)", { 255, 100, 100 }, Vars::Debug::Logging.Value);
		return CALL_ORIGINAL(entmsg);
	}
	...
	for (int i = 0; i < MAX_WEAPONS; i++)
	{
		...
		mCriticalStorage[i].m_iHandle = pLocal->m_hMyWeapons()[i].ToInt();
	...
	if (!pLocal || !pLocal->m_hMyWeapons())
	{
		SDK::Output("ProcessPacketEntities", "Failed to restore weapon crit data! (2)", { 255, 100, 100 }, Vars::Debug::Logging.Value);
		return bReturn;
	}
	...
	for (auto& [iSlot, tStorage] : mCriticalStorage)
	{
		...
		if (!pWeapon || pLocal->m_hMyWeapons()[iSlot].ToInt() != tStorage.m_iHandle) // don't restore onto a different weapon
			continue;
