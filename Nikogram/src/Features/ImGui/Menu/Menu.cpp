...
// bind currently loaded into the bind editor (Settings > Menu > Binds), file scope so bind removal elsewhere can keep it valid
static int s_iEditBind = DEFAULT_BIND;
static Bind_t s_tEditBind = {};

// whether iIndex is iBind or one of its children (the same set CBinds::RemoveBind removes)
static inline bool IsBindOrChild(int iIndex, int iBind)
{
	for (size_t i = 0; i <= F::Binds.m_vBinds.size() && iIndex > DEFAULT_BIND && iIndex < F::Binds.m_vBinds.size(); i++) // bounded in case of parent cycles
	{
		if (iIndex == iBind)
			return true;
		iIndex = F::Binds.m_vBinds[iIndex].m_iParent;
	}
	return false;
}

// index iIndex will have after iBind (and its children) is removed, DEFAULT_BIND if it is removed itself
static inline int RemapBindIndex(int iIndex, int iBind)
{
	if (iIndex <= DEFAULT_BIND || iIndex >= F::Binds.m_vBinds.size())
		return iIndex;
	if (IsBindOrChild(iIndex, iBind))
		return DEFAULT_BIND;

	int iNew = iIndex;
	for (int i = 0; i < iIndex; i++)
	{
		if (IsBindOrChild(i, iBind))
			iNew--;
	}
	return iNew;
}

// removes a bind and keeps the indices held by the menu (current bind, bind editor) pointing at the same binds
static inline void RemoveBindAndRemap(int iBind)
{
	if (iBind <= DEFAULT_BIND || iBind >= F::Binds.m_vBinds.size())
		return;

	int iCurrentBind = RemapBindIndex(ImGui::CurrentBind, iBind);
	int iEditBind = RemapBindIndex(s_iEditBind, iBind);
	int iEditParent = RemapBindIndex(s_tEditBind.m_iParent, iBind);

	F::Binds.RemoveBind(iBind);

	ImGui::CurrentBind = iCurrentBind;
	if (s_iEditBind != DEFAULT_BIND && iEditBind == DEFAULT_BIND)
	{
		s_iEditBind = DEFAULT_BIND;
		s_tEditBind = {};
	}
	else
	{
		s_iEditBind = iEditBind;
		s_tEditBind.m_iParent = iEditParent;
	}
}

// ActiveGroups is a 32 bit mask indexed by group, so it has to follow the groups when they are removed, inserted or moved
static inline uint32_t GroupMaskRemove(uint32_t uMask, int iIndex)
{
	if (iIndex < 0 || iIndex >= 32)
		return uMask;
	uint32_t uLow = uMask & ((1u << iIndex) - 1);
	uint32_t uHigh = iIndex < 31 ? (uMask >> (iIndex + 1)) << iIndex : 0u;
	return uLow | uHigh | 1u << 31; // unused bits default to active
}
static inline uint32_t GroupMaskInsert(uint32_t uMask, int iIndex, bool bValue)
{
	if (iIndex < 0 || iIndex >= 32)
		return uMask;
	uint32_t uLow = uMask & ((1u << iIndex) - 1);
	uint32_t uHigh = iIndex < 31 ? (uMask >> iIndex) << (iIndex + 1) : 0u;
	return uLow | uHigh | (bValue ? 1u << iIndex : 0u);
}
static inline void RemapActiveGroups(const std::function<uint32_t(uint32_t)>& fRemap)
{
	for (auto& iMask : Vars::ESP::ActiveGroups.Map | std::views::values)
		iMask = int(fRemap(uint32_t(iMask)));
	Vars::ESP::ActiveGroups.Value = int(fRemap(uint32_t(Vars::ESP::ActiveGroups.Value)));
}

// cached directory listings, refreshed at most once a second (or after invalidation) instead of walking the filesystem every frame
static std::unordered_map<std::string, std::pair<std::vector<std::filesystem::directory_entry>, double>> s_mDirectoryCache = {};
static inline void InvalidateDirectoryCache()
{
	s_mDirectoryCache.clear();
}
static inline const std::vector<std::filesystem::directory_entry>& GetDirectoryEntries(const std::string& sPath, bool* pRefreshed = nullptr)
{
	auto it = s_mDirectoryCache.find(sPath);
	double dTime = ImGui::GetTime();
	bool bRefresh = it == s_mDirectoryCache.end() || dTime - it->second.second > 1.0;
	if (pRefreshed)
		*pRefreshed = bRefresh;
	if (!bRefresh)
		return it->second.first;

	auto& [vEntries, dLastRefresh] = s_mDirectoryCache[sPath];
	vEntries.clear();
	dLastRefresh = dTime;

	std::error_code tError; // error_code overloads so a missing or locked folder doesn't throw inside the present hook
	for (auto itEntry = std::filesystem::directory_iterator(sPath, tError); !tError && itEntry != std::filesystem::directory_iterator(); itEntry.increment(tError))
		vEntries.push_back(*itEntry);
	return vEntries;
}
...
void CMenu::DrawMenu()
{
    ...
    static bool resetPending[Count] = {};
    if (applyReset) std::fill(std::begin(resetPending), std::end(resetPending), true);
    const float barHeight = H::Draw.Scale(20) + H::Draw.Scale(3) * 2;
    const float top = Workspace::TopTaskbar ? barHeight : 0.f;
    const float bottom = screen.y - (Workspace::TopTaskbar ? 0.f : barHeight);
    const auto launch = [&](int index) { open[index] = true; minimized[index] = false; focus = index; active = index; };
    Bind_t bind;
    if (!F::Binds.GetBind(CurrentBind, &bind)) CurrentBind = DEFAULT_BIND;

    // These overrides are scoped to the desktop: HUD editor windows keep their own geometry.
    PushStyleVar(ImGuiStyleVar_WindowPadding, { H::Draw.Scale(6), H::Draw.Scale(5) });
    PushStyleVar(ImGuiStyleVar_FramePadding, { H::Draw.Scale(4), H::Draw.Scale(2) });
    PushStyleVar(ImGuiStyleVar_ItemSpacing, { H::Draw.Scale(4), H::Draw.Scale(3) });
    PushStyleVar(ImGuiStyleVar_WindowTitleAlign, { 0.5f, 0.5f });
    PushFont(F::Render.FontSmall);

    const auto tabs = [&](const char* id, std::initializer_list<const char*> labels, int& selected) {
        PushID(id);
        int index = 0;
    ...
    {
        if (!open[i] || minimized[i]) continue;
        ImVec2 initialSize = i == Players ? ImVec2(830, 410) : i == Interface ? ImVec2(470, 390)
            : i == About ? ImVec2(300, 145) : ImVec2(730, 560);
        ImVec2 initialPos = i == Players ? ImVec2(30, 25) : i == Interface ? ImVec2(920, 320)
            : i == About ? ImVec2(840, 90) : ImVec2(285 + 22.f * i, 150 + 18.f * i);
        initialSize = initialSize * Vars::Menu::Scale.Value;
        initialSize.x = std::min(initialSize.x, screen.x - 12);
        initialSize.y = std::min(initialSize.y, bottom - top - 12);
        initialPos = initialPos * Vars::Menu::Scale.Value;
        initialPos.x = std::clamp(initialPos.x, 0.f, std::max(0.f, screen.x - initialSize.x));
        initialPos.y = std::clamp(initialPos.y + top, top, std::max(top, bottom - initialSize.y));
        SetNextWindowPos(initialPos, resetPending[i] ? ImGuiCond_Always : ImGuiCond_FirstUseEver);
        SetNextWindowSize(initialSize, resetPending[i] ? ImGuiCond_Always : ImGuiCond_FirstUseEver);
        // Constrain position BEFORE Begin calculates the window's clip/work rectangles.
        // Moving it afterwards leaves content clipped against the old off-screen position.
        if (!resetPending[i])
        {
            if (const ImGuiWindow* existing = FindWindowByName(ids[i]))
            {
                const ImVec2 size = existing->SizeFull;
                ImVec2 corrected = existing->Pos;
                const float maxX = std::max(0.f, screen.x - size.x);
                const float maxY = std::max(top, bottom - size.y);
                corrected.x = std::clamp(corrected.x, 0.f, maxX);
                corrected.y = std::clamp(corrected.y, top, maxY);
                if (Workspace::Snap && IsMouseReleased(ImGuiMouseButton_Left))
                {
                    const float distance = H::Draw.Scale(12);
                    if (corrected.x < distance) corrected.x = 0.f;
                    else if (maxX - corrected.x < distance) corrected.x = maxX;
                    if (corrected.y - top < distance) corrected.y = top;
                    else if (maxY - corrected.y < distance) corrected.y = maxY;
                }
                SetNextWindowPos(corrected, ImGuiCond_Always);
            }
        }
        resetPending[i] = false;
        SetNextWindowSizeConstraints({ std::min(H::Draw.Scale(i == About ? 250 : 410), screen.x - 12),
            std::min(H::Draw.Scale(100), bottom - top - 12) }, { screen.x - 4, bottom - top - 4 });
        if (focus == i) { SetNextWindowFocus(); focus = -1; }
        const bool visible = Begin(ids[i], &open[i], ImGuiWindowFlags_NoCollapse);
		if (visible && open[i])
		{
			const auto pos = GetWindowPos(), size = GetWindowSize();
			NikoPet::World.windows.push_back({ i, pos.x, pos.y, pos.x + size.x, pos.y + size.y });
		}
        if (IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows)) active = i;
        if (visible)
        {
            PushID(i);
            // A real title-bar item: temporarily leave the content clip rectangle.
            // Keep ImGui's native close button and dragging, with minimize beside X.
            ImGuiWindow* window = GetCurrentWindow();
            const ImRect title = window->TitleBarRect();
            const float closeLeft = title.Max.x - window->WindowBorderSize - GetStyle().FramePadding.x - GetFontSize();
            const float buttonSize = title.GetHeight();
            const ImRect minimizeRect(
                ImVec2(closeLeft - H::Draw.Scale(2) - buttonSize, title.Min.y),
                ImVec2(closeLeft - H::Draw.Scale(2), title.Max.y));
            const ImGuiID minimizeID = window->GetID("##TitleMinimize");
            PushClipRect(title.Min, title.Max, false);
            bool hovered = false, held = false;
            const ImGuiNavLayer previousLayer = window->DC.NavLayerCurrent;
            window->DC.NavLayerCurrent = ImGuiNavLayer_Menu;
            if (ItemAdd(minimizeRect, minimizeID))
            {
                if (ButtonBehavior(minimizeRect, minimizeID, &hovered, &held, ImGuiButtonFlags_NoNavFocus))
                {
                    minimized[i] = true;
                    active = -1;
                }
                ...
                if (hovered)
                    draw->AddRectFilled(minimizeRect.Min, minimizeRect.Max,
                        GetColorU32(held ? ImGuiCol_ButtonActive : ImGuiCol_ButtonHovered));
                draw->AddRect(minimizeRect.Min, minimizeRect.Max, GetColorU32(F::Render.Accent.Value));
                const float inset = std::max(3.f, buttonSize * 0.25f);
                const float underlineY = minimizeRect.Max.y - std::max(3.f, buttonSize * 0.2f);
                draw->AddLine(ImVec2(minimizeRect.Min.x + inset, underlineY),
                    ImVec2(minimizeRect.Max.x - inset, underlineY), GetColorU32(ImGuiCol_Text));
                // Border the neighboring native X to form a matching pair.
                draw->AddRect(ImVec2(closeLeft - H::Draw.Scale(1), title.Min.y),
                    ImVec2(title.Max.x - window->WindowBorderSize, title.Max.y), GetColorU32(F::Render.Accent.Value));
            }
            {
                ImGuiContext& g = *GImGui;
                IMGUI_TEST_ENGINE_ITEM_INFO(minimizeID, "Minimize to taskbar", g.LastItemData.StatusFlags);
            }
            window->DC.NavLayerCurrent = previousLayer;
            PopClipRect();
            if (hovered) SetTooltip("Minimize to taskbar");
            if (CurrentBind != DEFAULT_BIND)
            {
                TextColored(F::Render.Accent, "Editing bind: %s", bind.m_sName.c_str());
                SameLine();
                if (SmallButton("Stop")) CurrentBind = DEFAULT_BIND;
                Separator();
            }
            if (i == Main)
            {
                ...
                {
                    if (!search.empty()) MenuSearch(search);
                    else switch (mainTab)
                    {
            ...
            {
                if (BeginChild("PlayerPage", {}, ImGuiChildFlags_AlwaysUseWindowPadding)) MenuLogs(0);
                EndChild();
            }
            else if (i == Configs)
            {
                tabs("configs", { "Configs", "Binds", "Materials" }, configTab);
                if (BeginChild("ConfigPage", {}, ImGuiChildFlags_AlwaysUseWindowPadding)) MenuSettings(configTab);
                EndChild();
            }
            else if (i == Logs)
            {
            ...
            {
                TextUnformatted("nikogram");
                Separator();
                TextWrapped("Desktop workspace\nIndependent windows, compact controls and a taskbar.\nBased on Amalgam.");
            }
            PopID();
        }
        End();
    ...
    int openCount = 0;
    for (bool value : open) if (value) ++openCount;
    const float width = Workspace::CompactTaskbar ? std::min(screen.x, H::Draw.Scale(40 + 132 * openCount)) : screen.x;
	NikoPet::World.width = screen.x; NikoPet::World.height = screen.y;
	NikoPet::World.barWidth = width; NikoPet::World.barHeight = barHeight; NikoPet::World.top = Workspace::TopTaskbar;
    SetNextWindowPos({ 0, Workspace::TopTaskbar ? 0.f : screen.y - barHeight });
    SetNextWindowSize({ width, barHeight });
    PushStyleVar(ImGuiStyleVar_WindowPadding, { H::Draw.Scale(3), H::Draw.Scale(3) });
    // The normal minimum window height otherwise adds an empty strip below this compact bar.
    PushStyleVar(ImGuiStyleVar_WindowMinSize, { 0.f, 0.f });
    if (Begin("##NikogramTaskbar", nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoMove |
        ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoScrollbar |
        ImGuiWindowFlags_NoScrollWithMouse))
    {
        const ImVec2 launcherPos = GetCursorScreenPos();
        if (Button("##NikoLauncher", { H::Draw.Scale(26), H::Draw.Scale(20) })) OpenPopup("Launcher");
        if (const auto icon = F::Render.NikoLauncherIcon())
            GetWindowDrawList()->AddImage(icon, launcherPos + ImVec2(H::Draw.Scale(4), H::Draw.Scale(1)),
                launcherPos + ImVec2(H::Draw.Scale(22), H::Draw.Scale(19)), { 1, 0 }, { 0, 1 });
        if (IsPopupOpen("Launcher"))
            SetNextWindowPos({ 0, Workspace::TopTaskbar ? barHeight : screen.y - barHeight }, ImGuiCond_Always,
                { 0, Workspace::TopTaskbar ? 0.f : 1.f });
        PushStyleColor(ImGuiCol_PopupBg, F::Render.Background0.Value);
        PushStyleColor(ImGuiCol_Border, F::Render.Accent.Value);
        if (BeginPopup("Launcher"))
        {
            const float launcherWidth = H::Draw.Scale(264);
            const bool privacy = Vars::Visuals::UI::StreamerMode.Value >= Vars::Visuals::UI::StreamerModeEnum::Local;
            std::string playerName = "Local";
            const bool inGame = I::EngineClient && I::EngineClient->IsInGame();
            // Do not even request identifying strings when privacy is enabled.
            if (!privacy)
            {
                player_info_t info = {};
                const int local = I::EngineClient ? I::EngineClient->GetLocalPlayer() : 0;
                if (local > 0 && I::EngineClient->GetPlayerInfo(local, &info) && info.name[0])
                    playerName = F::PlayerUtils.GetPlayerName(local, info.name);
                else if (I::SteamFriends)
                {
                    const char* persona = I::SteamFriends->GetPersonaName();
                    if (persona && *persona) playerName = persona;
                }
            }
            const ImVec2 profilePos = GetCursorScreenPos();
            const float gap = H::Draw.Scale(3);
            const float avatarSize = H::Draw.Scale(62);
            const float lineHeight = H::Draw.Scale(20);
            const ImVec2 profileSize(launcherWidth, avatarSize);
            Dummy(profileSize);
            auto* launcherDraw = GetWindowDrawList();
            const ImU32 border = GetColorU32(F::Render.Accent.Value);
            const auto avatar = F::Render.LauncherAvatar(privacy);
            if (avatar)
                launcherDraw->AddImage(avatar, profilePos + ImVec2(1, 1), profilePos + ImVec2(avatarSize - 1, avatarSize - 1));
            else
                launcherDraw->AddText(profilePos + ImVec2(H::Draw.Scale(26), H::Draw.Scale(22)), GetColorU32(ImGuiCol_Text), "?");
            launcherDraw->AddRect(profilePos, profilePos + ImVec2(avatarSize, avatarSize), border);
            const std::string profileLines[] = {
                playerName, privacy ? "Privacy mode" : inGame ? "In game" : "Main menu",
                std::format("Config: {}", F::Configs.m_sCurrentConfig)
            };
            for (int row = 0; row < 3; ++row)
            {
                const ImVec2 p = profilePos + ImVec2(avatarSize + gap, row * (lineHeight + H::Draw.Scale(1)));
                const ImVec2 end(profilePos.x + launcherWidth, p.y + lineHeight);
                launcherDraw->AddRectFilled(p, end, GetColorU32(F::Render.Background0.Value));
                launcherDraw->AddRect(p, end, border);
                launcherDraw->PushClipRect(p + ImVec2(H::Draw.Scale(4), 1), end - ImVec2(H::Draw.Scale(3), 1), true);
                launcherDraw->AddText(p + ImVec2(H::Draw.Scale(4), (lineHeight - GetTextLineHeight()) / 2),
                    GetColorU32(ImGuiCol_Text), profileLines[row].c_str());
                launcherDraw->PopClipRect();
            }
            const ImVec2 bodyPos = GetCursorScreenPos();
            const float leftWidth = H::Draw.Scale(164);
            const float rightWidth = launcherWidth - leftWidth - gap;
            const float bodyHeight = H::Draw.Scale(238);
            launcherDraw->AddRectFilled(bodyPos, bodyPos + ImVec2(leftWidth, bodyHeight), GetColorU32(F::Render.Background0.Value));
            launcherDraw->AddRectFilled(bodyPos + ImVec2(leftWidth + gap, 0), bodyPos + ImVec2(launcherWidth, bodyHeight), GetColorU32(F::Render.Background0.Value));
            launcherDraw->AddRect(bodyPos, bodyPos + ImVec2(leftWidth, bodyHeight), border);
            launcherDraw->AddRect(bodyPos + ImVec2(leftWidth + gap, 0), bodyPos + ImVec2(launcherWidth, bodyHeight), border);
            const auto launcherButton = [&](const char* label, const char* icon, ImVec2 pos, ImVec2 size)
            {
                SetCursorScreenPos(pos);
                PushStyleColor(ImGuiCol_Button, F::Render.Background0.Value);
                const bool pressed = Button(std::format("##Launcher{}", label).c_str(), size);
                PopStyleColor();
                const bool hovered = IsItemHovered();
                if (hovered) SetMouseCursor(ImGuiMouseCursor_Hand);
                launcherDraw->AddRect(pos, pos + size, border);
                const float textX = icon ? H::Draw.Scale(30) : std::max(H::Draw.Scale(3), (size.x - CalcTextSize(label).x) / 2);
                launcherDraw->PushClipRect(pos + ImVec2(2, 1), pos + size - ImVec2(2, 1), true);
                if (icon)
                    launcherDraw->AddText(F::Render.IconFont, H::Draw.Scale(16), pos + ImVec2(H::Draw.Scale(7), (size.y - H::Draw.Scale(16)) / 2),
                        GetColorU32(F::Render.Accent.Value), icon);
                launcherDraw->AddText(pos + ImVec2(textX, (size.y - GetTextLineHeight()) / 2), GetColorU32(ImGuiCol_Text), label);
                launcherDraw->PopClipRect();
            ...
            const char* shortcuts[] = { "Full update", "Retry", "Console", "Reload materials", "Minimise all", "Reset layout", "Hide" };
            for (int row = 0; row < 7; ++row)
                if (launcherButton(shortcuts[row], nullptr,
                    bodyPos + ImVec2(leftWidth + gap, row * H::Draw.Scale(26)), { rightWidth, H::Draw.Scale(24) }))
                {
                    if (row == 0) I::EngineClient->ClientCmd_Unrestricted("cl_fullupdate");
                    else if (row == 1) I::EngineClient->ClientCmd_Unrestricted("retry");
                    else if (row == 2) I::EngineClient->ClientCmd_Unrestricted("toggleconsole");
                    else if (row == 3) { if (F::Materials.m_bLoaded) F::Materials.ReloadMaterials(); }
                    else if (row == 4)
                    {
                        for (int window = 0; window < Count; ++window) if (open[window]) minimized[window] = true;
                        active = -1;
                    }
                    else if (row == 5) resetLayout = true;
                    else { m_bIsOpen = false; I::MatSystemSurface->SetCursorAlwaysVisible(false); }
                    CloseCurrentPopup();
                }
            launcherDraw->AddText(bodyPos + ImVec2(H::Draw.Scale(7), bodyHeight - H::Draw.Scale(17)),
                GetColorU32(F::Render.Inactive.Value), "nikogram");
            // Register the full two-column panel without using cursor movement as size.
            SetCursorScreenPos(bodyPos);
            Dummy({ launcherWidth, bodyHeight });
            EndPopup();
        }
        PopStyleColor(2);
        const float tabWidth = std::min(H::Draw.Scale(128), std::max(28.f, (width - H::Draw.Scale(38)) / std::max(1, openCount) - H::Draw.Scale(4)));
        for (int i = 0; i < Count; ++i)
        {
            if (!open[i]) continue;
            SameLine();
            ...
            PopStyleColor();
            if (BeginPopupContextItem("WindowActions"))
            {
                if (MenuItem("Restore")) launch(i);
                if (MenuItem("Minimize")) minimized[i] = true;
                if (MenuItem("Close")) open[i] = false;
                EndPopup();
            }
            PopID();
        }
	...
	case 1:
	{
		if (BeginTable("DrawTable", 2))
		{
			...
			TableNextColumn();
			{
				...
				if (Section("Hitbox"))
				{
					FColoredDropdown(Vars::Visuals::Hitbox::BonesEnabled, { &Vars::Colors::TargetHitboxEdgeIgnoreZ, &Vars::Colors::TargetHitboxEdge, &Vars::Colors::BoneHitboxEdgeIgnoreZ, &Vars::Colors::BoneHitboxEdge, &Vars::Colors::TargetHitboxFaceIgnoreZ, &Vars::Colors::TargetHitboxFace, &Vars::Colors::BoneHitboxFaceIgnoreZ, &Vars::Colors::BoneHitboxFace }, FDropdownEnum::None);

					FColoredDropdown(Vars::Visuals::Hitbox::BoundsEnabled, { &Vars::Colors::BoundHitboxEdgeIgnoreZ, &Vars::Colors::BoundHitboxEdge, &Vars::Colors::BoundHitboxFaceIgnoreZ, &Vars::Colors::BoundHitboxFace }, FDropdownEnum::None);
			...
			TableNextColumn();
			{
				...
				{
					FColoredDropdown(Vars::Visuals::Prediction::PlayerPath, { &Vars::Colors::PlayerPathIgnoreZ, &Vars::Colors::PlayerPath }, FDropdownEnum::Left);
					FColoredDropdown(Vars::Visuals::Prediction::ProjectilePath, { &Vars::Colors::ProjectilePathIgnoreZ, &Vars::Colors::ProjectilePath }, FDropdownEnum::Right);
				...
				{
					FColoredDropdown(Vars::Visuals::Simulation::TrajectoryPath, { &Vars::Colors::TrajectoryPathIgnoreZ, &Vars::Colors::TrajectoryPath }, FDropdownEnum::Left);
					FColoredDropdown(Vars::Visuals::Simulation::ShotPath, { &Vars::Colors::ShotPathIgnoreZ, &Vars::Colors::ShotPath }, FDropdownEnum::Right);
					FColoredDropdown(Vars::Visuals::Simulation::SplashRadius, { &Vars::Colors::SplashRadiusIgnoreZ, &Vars::Colors::SplashRadius }, FDropdownEnum::None);
				...
				if (Vars::Debug::Options.Value)
				{
					if (Section("##Debug"))
					{
						...
						{
							FColoredDropdown(Vars::Visuals::Prediction::RealPath, { &Vars::Colors::RealPath, &Vars::Colors::RealPathIgnoreZ }, FDropdownEnum::None);
							...
							if (bApply)
							{
								...
								if (pLocal && pWeapon)
								{
									...
									if (bSetup)
									{
										...
										{
											auto pFireballDistance = H::ConVars.FindVar("tf_fireball_distance");
											flForwardRedirect = pFireballDistance ? pFireballDistance->GetFloat() : 0.f, flForwardCutoff = 1.f;
											break;
										}
										}
		...
		if (BeginTable("HvHTable", 2))
		{
			...
			TableNextColumn();
			{
				if (Section("Antiaim"))
				{
					...
					PopTransparent();
					const float afterSpinRow = GetCursorPosY();
			...
			TableNextColumn();
			{
				...
				if (Section("Fakelag"))
				{
					FDropdown(Vars::Fakelag::Fakelag, FDropdownEnum::Left);
					...
					FSlider(Vars::Backtrack::RealLag);
					if (Vars::Backtrack::RealLag.Value > 0) ImGui::TextWrapped("%s", RealLag::Status().c_str());
					if (Vars::Backtrack::RealLag.Value > 0) ImGui::TextWrapped("Log: %s", RealLag::DiagnosticPath().c_str());
				} EndSection();
				if (Vars::Debug::Options.Value)
				{
...
void CMenu::MenuVisuals(int iTab)
{
	...
	case 0:
	{
		...
		if (Section("Groups"))
		{
			...
			const ImVec2 groupRow = GetCursorPos();
			const float createHeight = CompactDropdownHeight(true);
			const float createWidth = FCalcTextSize("CREATE").x + H::Draw.Scale(20);

			PushDisabled(F::Groups.m_vGroups.size() >= sizeof(int) * 8); // for active groups flags
			{
				FSDropdown("Name", &sStaticName, {}, FDropdownEnum::Left | FSDropdownEnum::AutoUpdate, -H::Draw.Unscale(createWidth) - 8);

				PushDisabled(Disabled || sStaticName.empty());
				{
					SetCursorPos({ groupRow.x + GetWindowWidth() / 2 - GetStyle().WindowPadding.x * 1.5f - createWidth,
						groupRow.y + H::Draw.Scale(3) - GetStyle().WindowPadding.y });
					if (FButton("Create", FButtonEnum::None, { H::Draw.Unscale(createWidth), H::Draw.Unscale(createHeight) }))
					{
			...
			// Keep the right-hand dropdown anchored to the original row, not the button's padding offset.
			SetCursorPos(groupRow);
			Dummy({ 0, createHeight + H::Draw.Scale(3) });
			...
			const float groupTop = GetCursorPosY() + H::Draw.Scale(4);
			const float groupHeight = std::max(H::Draw.Scale(18), GetTextLineHeight() + H::Draw.Scale(4));
			auto fPositionToIndex = [&](ImVec2 vPos)
			{
				if (F::Groups.m_vGroups.empty()) return -1;
				int iIndex = int(floorf((vPos.y - groupTop) / groupHeight));
				iIndex = std::clamp(iIndex, 0, int(F::Groups.m_vGroups.size() - 1));
				return iIndex;
			};

			static int iDragging = -1;
			if (!IsMouseDown(ImGuiMouseButton_Left) || iDragging >= int(F::Groups.m_vGroups.size()))
				iDragging = -1;
			else if (iDragging != -1)
			{
				...
				if (iDragging != iTo)
				{
					...
					{
						const int iFrom = iDragging, iTarget = iTo;
						RemapActiveGroups([&](uint32_t uMask) { return GroupMaskInsert(GroupMaskRemove(uMask, iFrom), iTarget, iFrom < 32 && uMask & 1u << iFrom); });
					}
			...
			for (auto it = F::Groups.m_vGroups.begin(); it < F::Groups.m_vGroups.end();)
			{
				...
				ImVec2 vOriginalPos = { GetStyle().WindowPadding.x, groupTop + iGroup * groupHeight };
				const float deleteGap = H::Draw.Scale(4);
				float flWidth = GetWindowWidth() - GetStyle().WindowPadding.x * 2 - groupHeight - deleteGap;
				float flHeight = groupHeight;
				...
				if (iCurrentGroup != iGroup)
					GetWindowDrawList()->AddRectFilled(vDrawPos, vDrawPos + ImVec2(flWidth, flHeight), tColor, 0);
				else
				{
					ImColor tColor2 = { tColor.Value.x * 1.1f, tColor.Value.y * 1.1f, tColor.Value.z * 1.1f, tColor.Value.w };
					GetWindowDrawList()->AddRectFilled(vDrawPos, vDrawPos + ImVec2(flWidth, flHeight), tColor2, 0);
					...
					GetWindowDrawList()->AddRect(vDrawPos + ImVec2(flInset, flInset), vDrawPos + ImVec2(flWidth - flInset, flHeight - flInset), tColor2, 0, ImDrawFlags_None, H::Draw.Scale());
				}

				GetWindowDrawList()->AddRect(vDrawPos, vDrawPos + ImVec2(flWidth, flHeight), GetColorU32(ImGuiCol_TableBorderLight));

				// text + icons
				float flTextWidth = flWidth - H::Draw.Scale(12);
				SetCursorPos(vOriginalPos + ImVec2(H::Draw.Scale(6), (flHeight - GetTextLineHeight()) / 2));
				PushTransparent(!(iGroup < 32 && Vars::ESP::ActiveGroups.Value & 1 << iGroup), true); // ActiveGroups is a 32 bit mask
				{
					FText(TruncateText(tGroup.m_sName, flTextWidth).c_str());
				}
				PopTransparent(1, 1);

				SetCursorPos(vOriginalPos + ImVec2(flWidth + deleteGap, 0));
				PushID(iGroup);
				const ImVec2 deletePos = GetCursorScreenPos();
				bool bDelete = Button("##DeleteGroup", { flHeight, flHeight }), bDuplicate = false;
				if (IsItemHovered()) { SetMouseCursor(ImGuiMouseCursor_Hand); SetTooltip("Delete group"); }
				const float iconSize = std::min(H::Draw.Scale(14), flHeight - H::Draw.Scale(4));
				const ImVec2 iconExtent = F::Render.IconFont->CalcTextSizeA(iconSize, FLT_MAX, 0.f, ICON_MD_DELETE);
				GetWindowDrawList()->AddRect(deletePos, deletePos + ImVec2(flHeight, flHeight), GetColorU32(F::Render.Accent.Value));
				GetWindowDrawList()->AddText(F::Render.IconFont, iconSize,
					deletePos + (ImVec2(flHeight, flHeight) - iconExtent) * 0.5f, GetColorU32(F::Render.Accent.Value), ICON_MD_DELETE);
				PopID();

				SetCursorPos(vOriginalPos);
				bool bClicked = InvisibleButton(std::format("##GroupRow{}", iGroup).c_str(), { flWidth, flHeight });
				if (IsItemHovered())
				{
					SetMouseCursor(ImGuiMouseCursor_Hand);
					GetWindowDrawList()->AddRect(vDrawPos, vDrawPos + ImVec2(flWidth, flHeight), GetColorU32(F::Render.Accent.Value));
				}
				...
				if (bDelete)
				{
					...
					RemapActiveGroups([&](uint32_t uMask) { return GroupMaskRemove(uMask, iGroup); });
					if (iCurrentGroup > iGroup)
						iCurrentGroup--;
				}
				else if (bDuplicate)
				{
					...
					RemapActiveGroups([&](uint32_t uMask) { return GroupMaskInsert(uMask, iGroup + 1, iGroup < 32 && uMask & 1u << iGroup); });
					if (iCurrentGroup > iGroup)
						iCurrentGroup++;
				}
				else
			...
			SetCursorPos({ GetStyle().WindowPadding.x, groupTop });
			Dummy({ GetWindowWidth() - GetStyle().WindowPadding.x * 2, groupHeight * F::Groups.m_vGroups.size() });
		} EndSection();
			...
			&& BeginTable("VisualsESPTable", 2))
		{
			...
			TableNextColumn();
			{
				...
				if (Section("Misc"))
				{
					...
					if (FGroupPopupButton("OffscreenArrows"))
					{
					...
					FToggle("Pickup timer", &tGroup.m_bPickupTimer, FToggleEnum::Left);

					FToggle("Backtrack", &tGroup.m_iBacktrack, BacktrackEnum::Enabled, FToggleEnum::Left);
					if (FGroupPopupButton("Backtrack"))
					{
					...
					if (FGroupPopupButton("Trajectory"))
					{
						FDropdown("Flags", &tGroup.m_iTrajectory, { "Predict", "Radius", "Trace", "Sphere", "Path" }, { TrajectoryEnum::Predict, TrajectoryEnum::Radius, TrajectoryEnum::Trace, TrajectoryEnum::Sphere, TrajectoryEnum::Path }, FDropdownEnum::Multi | FDropdownEnum::NoSanitization);
					...
					if (FGroupPopupButton("Sightlines"))
					{
		...
		if (BeginTable("VisualsMiscTable", 2))
		{
			...
			TableNextColumn();
			{
				if (Section("UI"))
				{
					...
					FColoredDropdown(Vars::Visuals::UI::ChatTags, { &Vars::Colors::Local }, FDropdownEnum::Right);
				...
				if (Section("Effects"))
				{
					...
					FColoredDropdown(Vars::Visuals::Effects::SpellFootsteps, { &Vars::Colors::SpellFootstep }, FDropdownEnum::Right);
			...
			TableNextColumn();
			{
				if (Section("Removals"))
				{
					...
					FToggle(Vars::Visuals::Animations::Interpolation, FToggleEnum::Right);
				} EndSection();
				if (Section("Viewmodel"))
				{
					...
					PushTransparent(!Vars::Visuals::Viewmodel::CrosshairAim.Value);
					FToggle(Vars::Visuals::Viewmodel::CrosshairCooldown, FToggleEnum::Left);
					PopTransparent();
					PushTransparent(!Vars::Visuals::Viewmodel::ViewmodelAim.Value);
					FToggle(Vars::Visuals::Viewmodel::ViewmodelCooldown, FToggleEnum::Right);
					PopTransparent();
			...
			TableNextColumn();
			{
				if (Section("Settings"))
				{
					...
					if (FColorPicker("Outline / accent", &workspaceAccent, FColorPickerEnum::Left))
					{
						...
						Workspace::Accent[2] = workspaceAccent.b / 255.f;
					}
					if (FColorPicker("Background", &workspaceBackground, FColorPickerEnum::Right))
					{
						Workspace::Background[0] = workspaceBackground.r / 255.f;
			...
			TableNextColumn();
			{
				...
				if (Vars::Debug::Options.Value)
				{
					if (Section("##Debug"))
					{
						...
						if (FPopupButton("Debug", { 0, -5 }))
						{
							FSlider(Vars::Misc::Movement::AutoRocketJumpChokeGrounded, FSliderEnum::Left);
							FSlider(Vars::Misc::Movement::AutoRocketJumpChokeAir, FSliderEnum::Right);
							FSlider(Vars::Misc::Movement::AutoRocketJumpSkipGround, FSliderEnum::Left);
							FSlider(Vars::Misc::Movement::AutoRocketJumpSkipAir, FSliderEnum::Right);
							FSlider(Vars::Misc::Movement::AutoRocketJumpTimingOffset, FSliderEnum::Left);
							FSlider(Vars::Misc::Movement::AutoRocketJumpApplyAbove, FSliderEnum::Right);
...
void CMenu::MenuLogs(int iTab)
{
	...
	case 0:
	{
		if (Section("Players"))
		{
			if (I::EngineClient->IsInGame())
			{
				...
				auto fDrawPlayer = [&](const ListPlayer& tPlayer)
				{
					TableNextRow(ImGuiTableRowFlags_None, H::Draw.Scale(18));
					TableSetBgColor(ImGuiTableBgTarget_RowBg0, ColorByteToFloat(fGetTeamColor(tPlayer.m_iTeam, tPlayer.m_bAlive)));
					PushID(tPlayer.m_iUserID);
					TableSetColumnIndex(0);
					Selectable(std::format("{}##player", tPlayer.m_iIndex).c_str(), false,
						ImGuiSelectableFlags_SpanAllColumns, { 0, GetTextLineHeight() });
					...
					auto cell = [&](int column, const std::string& value)
					{
						...
						if (IsItemHovered() && CalcTextSize(value.c_str()).x > GetContentRegionAvail().x)
							SetTooltip("%s", value.c_str());
					};
					cell(1, std::to_string(tPlayer.m_iUserID));
					cell(2, tPlayer.m_sName);
					cell(3, std::to_string(tPlayer.m_iScore));
					cell(4, std::to_string(tPlayer.m_iDeaths));
					cell(5, tPlayer.m_bFake ? "BOT" : std::to_string(tPlayer.m_iPing));
					TableSetBgColor(ImGuiTableBgTarget_CellBg, tPlayer.m_bAlive ? IM_COL32(28, 110, 38, 255) : IM_COL32(55, 55, 55, 255), 6);
					cell(6, tPlayer.m_bAlive ? std::to_string(tPlayer.m_iHealth) : "DEAD");
					static const char* classes[] = { "-", "Scout", "Sniper", "Soldier", "Demoman", "Medic", "Heavy", "Pyro", "Spy", "Engineer" };
					cell(7, classes[std::clamp(tPlayer.m_iClass, 0, 9)]);
					TableSetColumnIndex(8);
					bool hasTag = false;
					if (tPlayer.m_bF2P) { TextUnformatted("F2P"); SameLine(0, H::Draw.Scale(6)); hasTag = true; }
					if (tPlayer.m_bLocal) { TextUnformatted("Local"); hasTag = true; }
					else if (tPlayer.m_bFriend) { TextUnformatted("Friend"); hasTag = true; }
					else if (tPlayer.m_bParty) { TextUnformatted("Party"); hasTag = true; }
					if (!tPlayer.m_bFake)
					{
						for (int tag : F::PlayerUtils.GetPlayerTags(tPlayer.m_uAccountID))
							if (auto entry = F::PlayerUtils.GetTag(tag))
							{
								if (hasTag) SameLine(0, H::Draw.Scale(6));
								TextColored(ColorByteToVec(entry->m_tColor), "%s", entry->m_sName.c_str());
								hasTag = true;
							}
					}
					// popups
					...
					if (FBeginPopup(std::format("RightClicked{}", tPlayer.m_iUserID).c_str()))
					{
						...
						{
							if (IsItemHovered() && IsMouseClicked(ImGuiMouseButton_Left)) // clicked, so holding the mouse doesn't send it every frame
							{
						...
						if (!tPlayer.m_bFake)
						{
							...
							if (FBeginMenu("Remove tag"))
							{
								int remove = -1;
								for (int tag : F::PlayerUtils.GetPlayerTags(tPlayer.m_uAccountID))
									if (auto entry = F::PlayerUtils.GetTag(tag))
										if (FSelectable(entry->m_sName.c_str())) remove = tag;
								if (remove >= 0) F::PlayerUtils.RemoveTag(tPlayer.m_uAccountID, remove, true, tPlayer.m_sName.c_str());
								ImGui::EndMenu();
							}
							if (FBeginMenu("Alias"))
							{
				...
				PushStyleVar(ImGuiStyleVar_CellPadding, { H::Draw.Scale(4), H::Draw.Scale(1) });
				PushStyleVar(ImGuiStyleVar_ItemSpacing, { 0, 0 });
				if (BeginTable("PlayerGrid", 9, ImGuiTableFlags_Borders | ImGuiTableFlags_Resizable | ImGuiTableFlags_SizingStretchProp))
				{
					TableSetupColumn("Index", ImGuiTableColumnFlags_WidthFixed, H::Draw.Scale(36));
					TableSetupColumn("UserID", ImGuiTableColumnFlags_WidthFixed, H::Draw.Scale(44));
					TableSetupColumn("Name", ImGuiTableColumnFlags_WidthStretch, 3.f);
					TableSetupColumn("Score", ImGuiTableColumnFlags_WidthFixed, H::Draw.Scale(38));
					TableSetupColumn("Deaths", ImGuiTableColumnFlags_WidthFixed, H::Draw.Scale(44));
					TableSetupColumn("Ping", ImGuiTableColumnFlags_WidthFixed, H::Draw.Scale(38));
					TableSetupColumn("Health", ImGuiTableColumnFlags_WidthFixed, H::Draw.Scale(44));
					TableSetupColumn("Class", ImGuiTableColumnFlags_WidthFixed, H::Draw.Scale(66));
					TableSetupColumn("Tags", ImGuiTableColumnFlags_WidthStretch, 1.5f);
					TableHeadersRow();
					for (int team : { 3, 2, 1, 0 })
						for (const auto& player : vPlayers)
							if (player.m_iTeam == team) fDrawPlayer(player);
					EndTable();
				}
				PopStyleVar(2);
			}
		...
		if (Section("Tags"))
		{
			...
			static int iPendingDelete = -1; // tag deletion is deferred until the tag list is no longer being iterated

			const float editorHeight = CompactDropdownHeight(true);
			const float actionWidth = editorHeight * 2 + H::Draw.Scale(8);
			auto vTable = WidgetTable(3, std::max(H::Draw.Scale(40), editorHeight + H::Draw.Scale(8)), { GetWindowWidth() / 2, GetWindowWidth() / 2 - actionWidth - GetStyle().WindowPadding.x });

			if (BeginWidgetTable(0, vTable))
			{
				const ImVec2 nameOrigin = GetCursorPos();
				FSDropdown("Name", &tTag.m_sName, {}, FDropdownEnum::Left | FSDropdownEnum::AutoUpdate, -14);

				PushDisabled(iID == F::PlayerUtils.TagToIndex(DEFAULT_TAG) || iID == F::PlayerUtils.TagToIndex(IGNORED_TAG)); // iID is a list index, not a tag id
				{
				...
				PopDisabled();
				// Draw the swatch after both fields so its item cannot move Type's baseline.
				const ImVec2 next = GetCursorPos();
				const float nameRight = nameOrigin.x + GetWindowWidth() / 2 - GetStyle().WindowPadding.x * 1.5f - H::Draw.Scale(14);
				const float typeLeft = GetWindowWidth() / 2 + GetStyle().WindowPadding.x * 0.5f;
				const float swatchLeft = (nameRight + typeLeft - H::Draw.Scale(10)) * 0.5f;
			...
			if (BeginWidgetTable(1, vTable))
			{
				...
				{
					SetCursorPosY(GetCursorPos().y + H::Draw.Scale(3));
			...
			if (BeginWidgetTable(2, vTable))
			{
				...
				const float buttonY = GetCursorPosY() + H::Draw.Scale(3) - GetStyle().WindowPadding.y;
				const float buttonSize = editorHeight / H::Draw.Scale(1);
				SetCursorPos({ GetWindowWidth() - GetStyle().WindowPadding.x - actionWidth, buttonY });
				PushDisabled(tTag.m_sName.empty());
				{
					bCreate = FButton(iID != -1 ? ICON_MD_SETTINGS : ICON_MD_ADD, FButtonEnum::None, { buttonSize, buttonSize }, 0, F::Render.IconFont);
				}
				PopDisabled();

				// clear button
				SetCursorPos({ GetWindowWidth() - GetStyle().WindowPadding.x - editorHeight, buttonY });
				bClear = FButton(ICON_MD_CLEAR, FButtonEnum::None, { buttonSize, buttonSize }, 0, F::Render.IconFont);
			...
			auto fDrawTag = [](std::vector<PriorityLabel_t>::iterator it, PriorityLabel_t& _tTag, int y)
			{
				...
				ImVec2 vOriginalPos = { !_tTag.m_bLabel ? GetStyle().WindowPadding.x : GetWindowWidth() * 2 / 3 + GetStyle().WindowPadding.x / 2, H::Draw.Scale(96 + 22 * y) };
				...
				// Square, contiguous table-like cells retain the configured tag colours.
				GetWindowDrawList()->AddRect(vDrawPos, vDrawPos + ImVec2(flWidth, flHeight), GetColorU32(ImGuiCol_TableBorderLight));
				if (!_tTag.m_bLabel)
					GetWindowDrawList()->AddLine(vDrawPos + ImVec2(flWidth / 2 - H::Draw.Scale(4), 0), vDrawPos + ImVec2(flWidth / 2 - H::Draw.Scale(4), flHeight), GetColorU32(ImGuiCol_TableBorderLight));
				GetWindowDrawList()->AddLine(vDrawPos + ImVec2(flWidth - H::Draw.Scale(28), 0), vDrawPos + ImVec2(flWidth - H::Draw.Scale(28), flHeight), GetColorU32(ImGuiCol_TableBorderLight));
				...
				if (!_tTag.m_bLabel)
				{
					SetCursorPos(vOriginalPos + ImVec2(flWidth / 2, H::Draw.Scale(4)));
				...
				if (!_tTag.m_bLocked)
				{
					...
					PushID(_iID);
					bDelete = SmallButton("x");
					PopID();
				}
				else
				{
					SetCursorPos(vOriginalPos + ImVec2(flWidth - H::Draw.Scale(22), H::Draw.Scale(3)));
				...
				SetCursorPos(vOriginalPos);
				bool bClicked = InvisibleButton(std::format("##TagRow{}", _iID).c_str(), { flWidth - H::Draw.Scale(28), flHeight });
				bool bPopup = IsItemClicked(ImGuiMouseButton_Right);
				...
				if (FBeginPopup(std::format("RightClicked{}", _iID).c_str()))
				{
					...
					PushDisabled(_iID == F::PlayerUtils.TagToIndex(DEFAULT_TAG) || _iID == F::PlayerUtils.TagToIndex(IGNORED_TAG));
					{
			...
			{
				const float x = label ? GetWindowWidth() * 2 / 3 + GetStyle().WindowPadding.x / 2 : GetStyle().WindowPadding.x;
				const float width = GetWindowWidth() * (label ? 1.f / 3 : 2.f / 3) - GetStyle().WindowPadding.x * 1.5f;
				const ImVec2 pos = GetDrawPos() + ImVec2(x, H::Draw.Scale(76));
				GetWindowDrawList()->AddRectFilled(pos, pos + ImVec2(width, H::Draw.Scale(20)), GetColorU32(ImGuiCol_TableHeaderBg));
				GetWindowDrawList()->AddRect(pos, pos + ImVec2(width, H::Draw.Scale(20)), GetColorU32(ImGuiCol_TableBorderStrong));
				SetCursorPos({ x + H::Draw.Scale(5), H::Draw.Scale(79) });
				FText(label ? "Labels" : "Priorities");
				if (!label)
				{
					SetCursorPos({ x + width / 2, H::Draw.Scale(79) }); FText("Value");
				}
			}
			...
			{
				// override for default tag (keep the comparator irreflexive)
				bool bDefaultA = std::distance(F::PlayerUtils.m_vTags.begin(), a.first) == F::PlayerUtils.TagToIndex(DEFAULT_TAG), bDefaultB = std::distance(F::PlayerUtils.m_vTags.begin(), b.first) == F::PlayerUtils.TagToIndex(DEFAULT_TAG);
				if (bDefaultA != bDefaultB)
					return bDefaultA;
			...
			// apply a pending deletion now that no iterators into m_vTags are in use
			if (iPendingDelete > -1 && iPendingDelete < F::PlayerUtils.m_vTags.size())
			{
				const int _iID = iPendingDelete;
				F::PlayerUtils.m_vTags.erase(F::PlayerUtils.m_vTags.begin() + _iID);
				F::PlayerUtils.m_bSave = true;
			...
			iPendingDelete = -1;
			SetCursorPos({ 0, H::Draw.Scale(72 + 22 * std::max(iPriorities, iLabels)) }); DebugDummy({ 0, H::Draw.Scale(28) });
		} EndSection();
		{
			PushDisabled(F::PlayerUtils.m_bLoad);
			{
				...
				if (FButton("Export", FButtonEnum::Fit | FButtonEnum::SameLine))
				{
					...
					std::ifstream fStream(F::Configs.m_sCorePath + "Players.json"); // no append mode, which would create an empty file if missing
					if (fStream.is_open())
					{
						std::string sString;
						{
						...
						SDK::Output("Nikogram", "Copied playerlist to clipboard", INFO_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG, ICON_MD_INFO);
					}
				}

				{
					...
					if (FButton("Import", FButtonEnum::Fit | FButtonEnum::SameLine))
					{
							...
							if (auto tSub = tRead.get_child_optional("Tags"))
							{
								for (auto& [sName, tChild] : *tSub)
								{
									...
									for (auto& tTag : tChild | std::views::values)
									{
										...
										if (iID < 0 || iID >= vTags.size() || !vTags[iID].m_bAssignable) // check against the imported tag list
											continue;

										if (!F::PlayerUtils.HasTag(uAccountID, iID, mPlayerTags))
											F::PlayerUtils.AddTag(uAccountID, iID, false, nullptr, mPlayerTags);
									}
								}
							}
						...
						{
							SDK::Output("Nikogram", "Failed to import playerlist", ERROR_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG, ICON_MD_CANCEL);
						}
					}
					...
					if (FBeginPopupModal("ImportPlayerlist"))
					{
						...
						for (int i = 0; i < vTags.size(); i++)
						{
							...
							{
								int iTo = iIDTo + 1;
								FDropdown(std::format("##{}", i).c_str(), &iTo, vEntries, vValues, FDropdownEnum::Right);
								iIDTo = iTo - 1;
							}
							PopTransparent();
						}
						...
						if (FButton("Import", FButtonEnum::Left))
						{
							...
							F::PlayerUtils.m_bSave = true;
							SDK::Output("Nikogram", "Imported playerlist", INFO_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG, ICON_MD_INFO);
				...
				if (FButton("Backup", FButtonEnum::Fit | FButtonEnum::SameLine))
				{
					try
					{
						// use the first unused backup name so an existing backup is never overwritten
						int iBackup = 1;
						while (std::filesystem::exists(F::Configs.m_sCorePath + std::format("Backup{}.json", iBackup)))
							iBackup++;
						std::filesystem::copy(
							F::Configs.m_sCorePath + "Players.json",
							F::Configs.m_sCorePath + std::format("Backup{}.json", iBackup)
						);
						SDK::Output("Nikogram", "Saved backup playerlist", INFO_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG, ICON_MD_INFO);
					}
					catch (...)
					{
						SDK::Output("Nikogram", "Failed to backup playerlist", ERROR_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG, ICON_MD_CANCEL);
					}
				}
			}
			PopDisabled();
	...
	// Settings moved to Visuals / Menu & HUD and Misc.
	case 1:
		break;
	// Output
	case 2:
	{
		if (Section("##Output", false, GetWindowHeight() - GetStyle().WindowPadding.y * 2))
		{
			...
			std::lock_guard tLock(m_tOutputMutex);
...
void CMenu::MenuSettings(int iTab)
{
		...
		if (BeginTable("ConfigSettingsTable", 2))
		{
			...
			*/
			...
			auto fDrawConfigs = [](std::string& sStaticName, bool bVisual = false)
			{
				...
				FSDropdown("Name", &sStaticName, {}, FSDropdownEnum::AutoUpdate, -H::Draw.Unscale(FCalcTextSize("CREATE").x + H::Draw.Scale(32)) - 36);
				PushDisabled(sStaticName.empty());
				{
					if (FButton("Create", FButtonEnum::Fit | FButtonEnum::SameLine | FButtonEnum::Compact, { 0, 32 }))
					{
						...
						if (!std::filesystem::exists(sPath + sStaticName + F::Configs.m_sConfigExtension, tError))
						{
							if (!bVisual)
				...
				bool bDefaultFound = false, bRefreshed = false;
				for (auto& tEntry : GetDirectoryEntries(sPath, &bRefreshed))
				{
					std::error_code tError;
					if (!tEntry.is_regular_file(tError) || tEntry.path().extension() != F::Configs.m_sConfigExtension)
						continue;

					std::string sName;
					try { sName = tEntry.path().filename().string(); }
					catch (...) { continue; } // name not representable in the narrow encoding
				...
				if (!bVisual)
				{
					if (!bDefaultFound && bRefreshed) // only when a fresh listing shows it missing, not every frame
					{
						if (F::Configs.EnsureDefaultConfig())
							InvalidateDirectoryCache();
					}
					std::sort(vConfigs.begin(), vConfigs.end(), [&](const auto& a, const auto& b) -> bool
					{
						// override for default config (keep the comparator irreflexive)
						bool bDefaultA = FNV1A::Hash32(a.second.c_str()) == FNV1A::Hash32Const("default"), bDefaultB = FNV1A::Hash32(b.second.c_str()) == FNV1A::Hash32Const("default");
						if (bDefaultA != bDefaultB)
							return bDefaultA;
				...
				for (auto& [entry, sConfigName] : vConfigs)
				{
					...
					SetCursorPos(vOriginalPos + ImVec2(H::Draw.Scale(2), H::Draw.Scale(1)));
					bool bLoad = CompactManagerIcon(bCurrentConfig ? ICON_MD_REFRESH : ICON_MD_DOWNLOAD);
					FTooltip(ICON_MD_ADD ICON_MD_FILE_UPLOAD_OFF, bNoSave && IsItemHovered(), 300.f, F::Render.IconFont);

					SetCursorPos({ H::Draw.Scale(43), vOriginalPos.y + H::Draw.Scale(4) });
					...
					int iOffset = 9;
					SetCursorPos({ GetWindowWidth() - H::Draw.Scale(iOffset += 25), vOriginalPos.y + H::Draw.Scale(1) });
					bool bDelete = CompactManagerIcon(ICON_MD_DELETE);

					SetCursorPos({ GetWindowWidth() - H::Draw.Scale(iOffset += 25), vOriginalPos.y + H::Draw.Scale(1) });
					bool bSave = CompactManagerIcon(ICON_MD_SAVE);
					...
					SetCursorPos(vOriginalPos); DebugDummy({ 0, H::Draw.Scale(22) });
				}
	...
	case 1:
	{
		...
		if (Section("Binds"))
		{
			int& iBind = s_iEditBind;
			Bind_t& tBind = s_tEditBind;
			int iPendingRemove = DEFAULT_BIND; // bind removal is deferred until the bind list is no longer being iterated
			...
			std::function<void(int, int)> fGetBinds = [&](int iParent, int x)
			{
				for (int _iBind = 0; _iBind < F::Binds.m_vBinds.size(); _iBind++)
				{
					...
					{
						float flInset = H::Draw.Scale(0.5f) - 0.5f;
						GetWindowDrawList()->AddRect(vDrawPos + ImVec2(flInset, flInset), vDrawPos + ImVec2(flWidth - flInset, flHeight - flInset), F::Render.Accent, 0.f, ImDrawFlags_None, H::Draw.Scale());
					}
					...
					{	// don't indent too much
						...
						SetCursorPos(vOriginalPos + ImVec2(-FCalcTextSize(sText.c_str()).x - H::Draw.Scale(10), H::Draw.Scale(4)));
						PushStyleColor(ImGuiCol_Text, F::Render.Inactive.Value);
						FText(sText.c_str());
						PopStyleColor();
					}
			...
			// apply a pending removal now that no references into the bind list are in use
			if (iPendingRemove != DEFAULT_BIND)
			{
				RemoveBindAndRemap(iPendingRemove);
				iDragging = -1;
			}
	...
	case 2:
	{
			...
			bTable = BeginTable("MaterialsTable", 2);
		{
			...
			if (Section("Manager"))
			{
				...
				// only copy what is displayed (not the whole material and its vmt); still a copy since materials can be removed while iterating
				struct MaterialEntry_t
				{
					std::string m_sName;
					bool m_bLocked;
				};
				std::vector<MaterialEntry_t> vMaterials;
				vMaterials.reserve(F::Materials.m_mMaterials.size());
				for (auto& tMaterial : F::Materials.m_mMaterials | std::views::values)
					vMaterials.push_back({ tMaterial.m_sName, tMaterial.m_bLocked });

				std::sort(vMaterials.begin(), vMaterials.end(), [&](const auto& a, const auto& b) -> bool
				{
					// override for none material (keep the comparator irreflexive)
					bool bNoneA = FNV1A::Hash32(a.m_sName.c_str()) == FNV1A::Hash32Const("None"), bNoneB = FNV1A::Hash32(b.m_sName.c_str()) == FNV1A::Hash32Const("None");
					if (bNoneA != bNoneB)
						return bNoneA;
				...
				for (auto& tMaterial : vMaterials)
				{
					...
					SetCursorPos({ H::Draw.Scale(17), vOriginalPos.y + H::Draw.Scale(4) });
					...
					{
						SetCursorPos({ GetWindowWidth() - H::Draw.Scale(iOffset += 25), vOriginalPos.y + H::Draw.Scale(1) });
						if (CompactManagerIcon(ICON_MD_DELETE))
			...
			if (bTable)
			{
				...
				if (sCurrentMaterial.length())
				{
					...
					if (Section("Editor", 0, GetWindowHeight() - GetStyle().WindowPadding.y * 2, true))
					{
						...
						if (static bool bLanguageSet = false; !bLanguageSet) // setting the language can recolorize the whole text, so only do it once
						{
...
void CMenu::MenuSearch(std::string sSearch)
{
		...
		uCurrHash != uStaticHash)
	{
		...
		std::transform(sSearch2.begin(), sSearch2.end(), sSearch2.begin(), [](unsigned char c) { return char(std::tolower(c)); });
		...
		for (auto& pBase : G::Vars)
		{
			...
			for (auto pSearch : vSearch)
			{
				...
				std::transform(sSearch3.begin(), sSearch3.end(), sSearch3.begin(), [](unsigned char c) { return char(std::tolower(c)); });
...
void CMenu::Render()
{
	...
	{
		NikoPet::Draw();
		m_bWindowHovered |= NikoPet::Hovered;
		F::Render.Cursor = GetMouseCursor();
	}
	PopFont();
}
