...
Enum(FButton, None = 0, Left = 1 << 0, Right = 1 << 1, Fit = 1 << 2, SameLine = 1 << 3, NoUpper = 1 << 4, Compact = 1 << 6);
...
namespace ImGui
{
	...
	inline std::vector<Row_t> vRowSizesRegular = {}, vRowSizesPopup = {};
	inline std::vector<Row_t>* pRowSizes = &vRowSizesRegular;
	...
	inline void RenderTwoToneBackground(float flSize, ImU32 uTitle, ImU32 uBackground, bool bHorizontal = false)
	{
		...
		if (!bHorizontal)
		{
			pDrawList->AddRectFilled(vDrawPos, vDrawPos + ImVec2(vSize.x, flSize), uTitle, 0.f, ImDrawFlags_RoundCornersTop);
			pDrawList->AddRectFilled(vDrawPos + ImVec2(0, flSize), vDrawPos + vSize, uBackground, 0.f, ImDrawFlags_RoundCornersBottom);
		}
		else
		{
			pDrawList->AddRectFilled(vDrawPos, vDrawPos + ImVec2(flSize, vSize.y), uTitle, 0.f, ImDrawFlags_RoundCornersLeft);
			pDrawList->AddRectFilled(vDrawPos + ImVec2(flSize, 0), vDrawPos + vSize, uBackground, 0.f, ImDrawFlags_RoundCornersRight);
		}
	}
	inline void RenderTwoToneBackground(float flSize, ImU32 uTitle, ImU32 uBackground, ImU32 uBorder, float flInset = H::Draw.Scale(), bool bVertical = true, bool bTwoToneBorder = true)
	{
		...
		{
			if (!bTwoToneBorder)
				pDrawList->AddRectFilled(vDrawPos + ImVec2(flInset, flInset), vDrawPos + ImVec2(vSize.x - flInset, flSize), uTitle, 0.f, ImDrawFlags_RoundCornersTop);
			else
			{
				pDrawList->AddRectFilled(vDrawPos + ImVec2(flInset, flInset), vDrawPos + ImVec2(vSize.x - flInset, flSize - H::Draw.Scale()), uTitle, 0.f, ImDrawFlags_RoundCornersTop);
				pDrawList->AddRectFilled(vDrawPos + ImVec2(flInset, flSize - H::Draw.Scale()), vDrawPos + ImVec2(vSize.x - flInset, flSize), uBorder);
			}
			pDrawList->AddRectFilled(vDrawPos + ImVec2(flInset, flSize), vDrawPos + vSize - ImVec2(flInset, flInset), uBackground, 0.f, ImDrawFlags_RoundCornersBottom);
		}
		else
		{
			if (!bTwoToneBorder)
				pDrawList->AddRectFilled(vDrawPos + ImVec2(flInset, flInset), vDrawPos + ImVec2(flSize, vSize.y - flInset), uTitle, 0.f, ImDrawFlags_RoundCornersLeft);
			else
			{
				pDrawList->AddRectFilled(vDrawPos + ImVec2(flInset, flInset), vDrawPos + ImVec2(flSize - H::Draw.Scale(), vSize.y - flInset), uTitle, 0.f, ImDrawFlags_RoundCornersLeft);
				pDrawList->AddRectFilled(vDrawPos + ImVec2(flSize - H::Draw.Scale(), flInset), vDrawPos + ImVec2(flSize, vSize.y - flInset), uBorder);
			}
			pDrawList->AddRectFilled(vDrawPos + ImVec2(flSize, flInset), vDrawPos + vSize - ImVec2(flInset, flInset), uBackground, 0.f, ImDrawFlags_RoundCornersRight);
		}
	...
	inline void AddSteppedRect(ImVec2 vPos, ImVec2 vPosMin, ImVec2 vPosMax, ImVec2 vClipMin, ImVec2 vClipMax, float flVMin, float flVMax, float flStep, ImU32 uPrimary, ImU32 uSecondary, float flStepWidth)
	{
		...
		if (bValid && (iSteps < 21 || (vPosMax.x - vPosMin.x) / iSteps > 6.f))
		{
			...
			// steps are relative to the minimum, matching how the slider snaps
			float flMin = flVMin, max = flVMin + (flVMax - flVMin) - fnmodf(flVMax - flVMin + flStep / 2, flStep) + flStep / 2;
	...
	inline std::string TruncateText(const std::string& sText, int iPixels, ImFont* pFont = nullptr, const char* sEnd = "...", int iMin = 0, int* pLength = nullptr)
	{
		...
		int i = 0; while (i < sText.length())
		{
			// advance by whole UTF-8 characters so a multi-byte character is never cut in half
			int iNext = i + 1;
			while (iNext < sText.length() && (sText[iNext] & 0xC0) == 0x80)
				iNext++;
			if (i >= iMin && FCalcTextSize(std::format("{}{}{}", sTruncated, std::string_view(sText).substr(i, iNext - i), sEnd).c_str(), pFont).x >= iPixels)
				break;
			sTruncated.append(sText, i, iNext - i);
			i = iNext;
		}
		if (i)
			sTruncated += sEnd;
	...
	inline void FTooltip(const char* sTooltip, bool bCondition = IsItemHovered(), float flWrapWidth = 300, ImFont* pFont = F::Render.FontSmall, ImVec2* pPosOverride = nullptr, ImVec2* pSizeOverride = nullptr)
	{
		if (bCondition)
		{
			...
			pDrawList->AddRect(vPos + ImVec2(flInset, flInset), vPos + ImVec2(vText.x - flInset, vText.y - flInset), F::Render.Background2, 0.f, ImDrawFlags_None, H::Draw.Scale());
			...
			pDrawList->AddRectFilled(vPos + ImVec2(flInset, flInset), vPos + ImVec2(vText.x-flInset, vText.y - flInset), F::Render.Background1p5, 0.f);
			flInset = 1;
	...
	inline bool CompactManagerIcon(const char* icon)
	{
		...
		draw->AddRect(pos, pos + ImVec2(size, size), GetColorU32(F::Render.Accent.Value));
		const ImVec2 extent = F::Render.IconFont->CalcTextSizeA(glyphSize, FLT_MAX, 0.f, icon);
		ImVec4 color = F::Render.Accent.Value;
		color.w *= Transparent || Disabled ? 0.5f : 1.f;
		draw->AddText(F::Render.IconFont, glyphSize, pos + (ImVec2(size, size) - extent) * 0.5f, GetColorU32(color), icon);
	...
	inline bool FGroupPopupButton(const char* label)
	{
		const float size = H::Draw.Scale(12);
		const ImVec2 rowBottom = GetItemRectMax();
		const float panelWidth = GetWindowWidth();
		const float panelLeft = GetWindowPos().x;
		SameLine(GetWindowWidth() - GetStyle().WindowPadding.x - size - H::Draw.Scale(2));
		SetCursorPosY(GetCursorPosY() + H::Draw.Scale(3.5f));
		const ImVec2 pos = GetCursorScreenPos();
		const bool active = IsPopupOpen(label);
		PushID(label);
		const bool pressed = InvisibleButton("##Expand", { size, size });
		if (IsItemHovered()) SetMouseCursor(ImGuiMouseCursor_Hand);
		PopID();
		auto* draw = GetWindowDrawList();
		if (active || IsItemHovered()) draw->AddRectFilled(pos, pos + ImVec2(size, size), GetColorU32(F::Render.Background2.Value));
		const ImU32 accent = GetColorU32(F::Render.Accent.Value);
		draw->AddRect(pos, pos + ImVec2(size, size), accent);
		const float y1 = size * (active ? 0.6f : 0.4f), y2 = size * (active ? 0.4f : 0.6f);
		draw->AddLine(pos + ImVec2(size * 0.3f, y1), pos + ImVec2(size * 0.5f, y2), accent);
		draw->AddLine(pos + ImVec2(size * 0.5f, y2), pos + ImVec2(size * 0.7f, y1), accent);
		if (pressed) OpenPopup(label);
		SetNextWindowPos({ panelLeft, rowBottom.y + H::Draw.Scale(2) }, ImGuiCond_Appearing);
		SetNextWindowSize({ panelWidth, 0 }, ImGuiCond_Appearing);
		PushStyleVar(ImGuiStyleVar_WindowPadding, { H::Draw.Scale(6), H::Draw.Scale(4) });
		const bool open = BeginPopup(label, ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoBackground);
		PopStyleVar();
		if (open) RenderBackground(F::Render.Background0, F::Render.Accent);
		return open;
	}

	inline bool FSelectable(const char* sLabel, ImVec4* pColor, float flRounding = 0.f, bool bSelected = false, ImGuiSelectableFlags iFlags = ImGuiSelectableFlags_None, const ImVec2& vSize = {})
	{
	...
	inline bool FSelectable(const char* sLabel, ImVec4 tColor = { 0.2f, 0.6f, 0.85f, 1.f }, float flRounding = 0.f, bool bSelected = false, ImGuiSelectableFlags iFlags = ImGuiSelectableFlags_None, const ImVec2& vSize = {})
	{
	...
	inline std::unordered_map<uint32_t, float> mLastHeights = {};
	inline std::vector<uint32_t> vStoredLabels = {};
	inline bool Section(const char* sLabel, float flPaddingMod = 0.f, float flMinHeight = 28.f, bool bForceHeight = false)
	{
		// Scope cached heights to this window/table, not every matching title.
		uint32_t uHash = GetID(sLabel);
		...
		if (sLabel[0] != '#')
		{
			...
			const ImVec2 titlePadding(H::Draw.Scale(3), H::Draw.Scale(2));
			const ImVec2 titleMin = GetCursorScreenPos();
			const std::string title = TruncateText(StripDoubleHash(sLabel),
				std::max(1.f, GetContentRegionAvail().x - titlePadding.x * 2));
			// Keep the existing divider/control geometry, but center the label in
			// the full header strip rather than offsetting it from content padding.
			const float textHeight = GetTextLineHeight();
			const ImVec2 titleMax = titleMin + ImVec2(0, textHeight + titlePadding.y * 2);
			// The section's outer frame supplies the header's top and sides.
			// Draw its bottom edge across the full section, beyond the content padding.
			// OuterRectClipped still respects the parent window/scroll viewport.
			auto* sectionWindow = GetCurrentWindow();
			auto* sectionDraw = GetWindowDrawList();
			const float borderInset = H::Draw.Scale(1);
			sectionDraw->PushClipRect(sectionWindow->OuterRectClipped.Min, sectionWindow->OuterRectClipped.Max, false);
	...
	inline void EndSection()
	{
		...
		auto* sectionWindow = GetCurrentWindow();
		const float flHeight = SectionLayout::ContentHeight(sectionWindow);
		if (!sectionWindow->SkipItems && flHeight > 0.f)
			mLastHeights[uHash] = flHeight;
	...
	inline bool FButton(const char* sLabel, int iFlags = FButtonEnum::None, ImVec2 vSize = { 0, 30 }, int iSizeOffset = 0, ImFont* pFont = nullptr, bool* pHovered = nullptr)
	{
		...
		{
			std::transform(sLabel2.begin(), sLabel2.end(), sLabel2.begin(), [](unsigned char c) { return char(std::toupper(c)); });
		...
		ImVec2 vOriginalPos = GetCursorPos();
		const float topPadding = iFlags & FButtonEnum::Compact ? H::Draw.Scale(3) : GetStyle().WindowPadding.y;
		DebugShift({ 0, topPadding });
		...
		SetCursorPos(vOriginalPos);
		AddRowSize(vOriginalPos, vSize + ImVec2(0, topPadding));
		DebugDummy({ vSize.x, GetRowSize(vSize.y + topPadding) });
	...
	{
		if (Transparent || Disabled) PushStyleVar(ImGuiStyleVar_Alpha, 0.5f);
		ImVec2 size = { GetWindowWidth() - GetStyle().WindowPadding.x * 2, 0 };
		if (iFlags & (FToggleEnum::Left | FToggleEnum::Right))
			size.x = GetWindowWidth() / 2 - GetStyle().WindowPadding.x * 1.5f;
		if (iFlags & FToggleEnum::Right) SameLine(size.x + GetStyle().WindowPadding.x * 2);
		const ImVec2 origin = GetCursorPos();
		auto lines = WrapText(StripDoubleHash(sLabel), size.x - H::Draw.Scale(20));
		...
		if (changed) *pVar = !*pVar;
		const bool checked = (iFlags & FToggleEnum::Invert) ? !*pVar : *pVar;
		...
		SetCursorPos(origin);
		AddRowSize(origin, size);
		DebugDummy({ size.x, GetRowSize(size.y) });
		if (Transparent || Disabled) PopStyleVar();
		return changed;
	}
	...
	inline bool FSlider(const char* sLabel, float* pVar1, float* pVar2, float flMin, float flMax, float flStep = 1.f, const char* fmt = "%g", int iFlags = FSliderEnum::None, bool* pHovered = nullptr)
	{
		...
		// guard against a zero step or an empty range, which would otherwise divide by zero
		bool bStep = flStep > 0.f;
		float flRange = flMax != flMin ? flMax - flMin : 1.f;
			...
			else
			{
				...
				if (bEnter)
				{
					try
					{
						...
						if (!(iFlags & FSliderEnum::Precision) && bStep)
							pVar = flMin + (pVar - flMin) - fnmodf(pVar - flMin - flStep / 2, flStep) + flStep / 2;
		...
		if (!Disabled && bWithin)
			SetMouseCursor(ImGuiMouseCursor_Hand);
		ImVec2 vMouse = GetMousePos();
		float flMousePerc = (vMouse.x - (vDrawPos.x + vMins.x)) / ((vDrawPos.x + vMaxs.x) - (vDrawPos.x + vMins.x)) + (flStep / 2) / flRange;
		if (pVar2)
		{
			float flLowerPerc = std::clamp((flSVar1 - flMin) / flRange, 0.f, 1.f), flUpperPerc = std::clamp((flSVar2 - flMin) / flRange, 0.f, 1.f);
			...
			pDrawList->AddRectFilled(vDrawPos + ImVec2(flLowerPos, vMins.y + H::Draw.Scale(1)) - ImVec2(H::Draw.Scale(3), H::Draw.Scale(3)), vDrawPos + ImVec2(flLowerPos, vMins.y + H::Draw.Scale(1)) + ImVec2(H::Draw.Scale(3), H::Draw.Scale(3)), tAccent);
			pDrawList->AddRectFilled(vDrawPos + ImVec2(flUpperPos, vMins.y + H::Draw.Scale(1)) - ImVec2(H::Draw.Scale(3), H::Draw.Scale(3)), vDrawPos + ImVec2(flUpperPos, vMins.y + H::Draw.Scale(1)) + ImVec2(H::Draw.Scale(3), H::Draw.Scale(3)), tAccent);
			...
			if (!Disabled)
			{
				if (bWithin && !ActiveMap[uHash])
				{
					...
					pDrawList->AddRectFilled(vDrawPos + ImVec2((iVar == 1 ? flLowerPos : flUpperPos), vMins.y + H::Draw.Scale(1)) - ImVec2(H::Draw.Scale(5), H::Draw.Scale(5)), vDrawPos + ImVec2((iVar == 1 ? flLowerPos : flUpperPos), vMins.y + H::Draw.Scale(1)) + ImVec2(H::Draw.Scale(5), H::Draw.Scale(5)), tTransparent);
				}
				else if (ActiveMap[uHash] && IsMouseDown(ImGuiMouseButton_Left))
				{
					...
					flVar = std::clamp(bStep ? flVar - fnmodf(flVar - flMin, flStep) : flVar, !bVar1 ? flSVar1 + flStep : flMin, bVar1 ? flSVar2 - flStep : flMax);
					pDrawList->AddRectFilled(vDrawPos + ImVec2((bVar1 ? flLowerPos : flUpperPos), vMins.y + H::Draw.Scale(1)) - ImVec2(H::Draw.Scale(5), H::Draw.Scale(5)), vDrawPos + ImVec2((bVar1 ? flLowerPos : flUpperPos), vMins.y + H::Draw.Scale(1)) + ImVec2(H::Draw.Scale(5), H::Draw.Scale(5)), tTransparent);
				}
				else
		...
		{
			float flPercent = std::clamp((flSVar1 - flMin) / flRange, 0.f, 1.f);
			...
			pDrawList->AddRectFilled(vDrawPos + ImVec2(flPos, vMins.y + H::Draw.Scale(1)) - ImVec2(H::Draw.Scale(3), H::Draw.Scale(3)), vDrawPos + ImVec2(flPos, vMins.y + H::Draw.Scale(1)) + ImVec2(H::Draw.Scale(3), H::Draw.Scale(3)), tAccent);
			...
			if (!Disabled)
			{
				if (bWithin && !ActiveMap[uHash])
				{
					if (IsMouseClicked(ImGuiMouseButton_Left))
						ActiveMap[uHash] = 1;
					pDrawList->AddRectFilled(vDrawPos + ImVec2(flPos, vMins.y + H::Draw.Scale(1)) - ImVec2(H::Draw.Scale(5), H::Draw.Scale(5)), vDrawPos + ImVec2(flPos, vMins.y + H::Draw.Scale(1)) + ImVec2(H::Draw.Scale(5), H::Draw.Scale(5)), tTransparent);
				}
				else if (ActiveMap[uHash] && IsMouseDown(ImGuiMouseButton_Left))
				{
					flSVar1 = flMin + (flMax - flMin) * flMousePerc;
					flSVar1 = std::clamp(bStep ? flSVar1 - fnmodf(flSVar1 - flMin, flStep) : flSVar1, flMin, flMax);
					pDrawList->AddRectFilled(vDrawPos + ImVec2(flPos, vMins.y + H::Draw.Scale(1)) - ImVec2(H::Draw.Scale(5), H::Draw.Scale(5)), vDrawPos + ImVec2(flPos, vMins.y + H::Draw.Scale(1)) + ImVec2(H::Draw.Scale(5), H::Draw.Scale(5)), tTransparent);
				}
				else
					ActiveMap[uHash] = false;
	...
	inline bool FBeginCompactCombo(const char* label, const char* preview, ImGuiComboFlags flags)
	{
		auto* ownerDraw = GetWindowDrawList();
		const ImVec2 framePos = GetCursorScreenPos();
		const ImVec2 frameSize(CalcItemWidth(), GetFrameHeight());
		// Selectable extends half the eight-unit row spacing below its content.
		// Matching popup padding to that edge avoids an empty footer.
		PushStyleVar(ImGuiStyleVar_WindowPadding, { GetStyle().WindowPadding.x, H::Draw.Scale(4) });
		const bool open = BeginCombo(label, preview, flags);
		PopStyleVar();
		if (open) ownerDraw->AddRect(framePos, framePos + frameSize, GetColorU32(F::Render.Accent.Value));
		return open;
	}

	inline void FFinishCompactComboLayout()
	{
		// Overlay labels restore the cursor to the next row, beyond actual content.
		// Submit a zero-size item at the existing content edge, not at that cursor:
		// this validates the layout without adding a blank footer.
		auto* window = GetCurrentWindow();
		SetCursorScreenPos({ window->DC.CursorStartPos.x, window->DC.CursorMaxPos.y });
		Dummy({ 0, 0 });
	}

	// Compact bordered options, scoped to dropdowns (not context menus).
	inline void FDropdownIcon(const char* icon, ImVec4 color = { 1, 1, 1, -1 })
	{
		// Decorative glyphs belong to the selectable row; their font line box
		// must not extend popup content below the final option.
		if (Transparent || Disabled)
			PushStyleVar(ImGuiStyleVar_Alpha, 0.5f);
		if (color.w >= 0.f)
			PushStyleColor(ImGuiCol_Text, color);
		PushFont(F::Render.IconFont);
		GetWindowDrawList()->AddText(GetFont(), GetFontSize(), GetCursorScreenPos(), GetColorU32(ImGuiCol_Text), icon);
		PopFont();
		if (color.w >= 0.f)
			PopStyleColor();
		if (Transparent || Disabled)
	...
	inline float AttachedDropdownWidth = 0.f;
	inline ImVec2 AttachedDropdownOrigin;
	inline float AttachedDropdownHeight = 0.f;
	inline constexpr int AttachedColorPicker = 1 << 9;

	inline float CompactDropdownHeight(bool hasTitle)
	{
		const float valueHeight = GetTextLineHeight();
		PushFont(F::Render.FontSmall);
	...
	inline bool FDropdown(const char* sLabel, int* pVar, std::vector<const char*> vEntries, std::vector<int> vValues = {}, int iFlags = FDropdownEnum::None, int iSizeOffset = 0, const char* sDefaultPreview = "None", bool* pHovered = nullptr, int* pModified = nullptr)
	{
		...
		ImVec2 vOriginalPos = GetCursorPos();
		DebugShift({ 0, H::Draw.Scale(3) });
		if (leadingWidth > 0.f)
		{
			AttachedDropdownOrigin = GetCursorPos();
			AttachedDropdownHeight = vSize.y;
			vSize.x = std::max(H::Draw.Scale(30), vSize.x - leadingWidth);
			SetCursorPosX(GetCursorPosX() + leadingWidth);
		}

		if (Disabled)
		{	// lol
			const ImVec2 framePos = GetCursorPos();
			Button("##", vSize);
			SetCursorPos(framePos);
		}

		PushStyleVar(ImGuiStyleVar_FramePadding, { 0.f, std::max(0.f, (vSize.y - GetTextLineHeight()) * 0.5f) });
		PushItemWidth(vSize.x);

		bool bActive = FBeginCompactCombo(std::format("##{}", sLabel).c_str(), "", ImGuiComboFlags_CustomPreview | ImGuiComboFlags_NoArrowButton | ImGuiComboFlags_HeightLarge);
		if (bActive)
		{
			DebugDummy({ 0, H::Draw.Scale(2) });

			PushStyleVar(ImGuiStyleVar_ItemSpacing, { 0, H::Draw.Scale(8) });
			int i = 0; for (auto& sEntry : vEntries)
			{
				if (FNV1A::Hash32(sEntry) == FNV1A::Hash32Const("##Divider"))
				{
				...
				else
				{
					...
					if (FDropdownSelectable(std::format("##{}{}", sEntry, i).c_str(), nullptr, 0, *pVar == vValues[i]))
					...
					{
						SetCursorPos(vOriginalPos2 + ImVec2(H::Draw.Scale(15), H::Draw.Scale(-1)));
						FDropdownIcon(vValues[i] == -1 ? ICON_MD_ADD_CIRCLE : ICON_MD_REMOVE_CIRCLE);
					}
					SetCursorPos(vOriginalPos3);
				}
				i++;
		...
		{
			ImVec2 vOriginalPos2 = GetCursorPos();
			const float previewTop = vOriginalPos2.y - GetStyle().FramePadding.y;
			const float valueY = bTitle ? previewTop + vSize.y - H::Draw.Scale(4) - GetTextLineHeight()
			...
			if (bTitle)
			{
				...
				TextUnformatted(TruncateText(sPreview, vSize.x - H::Draw.Scale(30)).c_str());
		...
		SetCursorPos(vOriginalPos);
		AddRowSize(vOriginalPos, vSize + ImVec2(leadingWidth, H::Draw.Scale(3)));
		DebugDummy({ vSize.x + leadingWidth, GetRowSize(vSize.y + H::Draw.Scale(3)) });
	...
	inline bool FSDropdown(const char* sLabel, std::string* pVar, std::vector<const char*> vEntries = {}, int iFlags = FDropdownEnum::None, int iSizeOffset = 0, bool* pHovered = nullptr)
	{
		...
		if (FBeginCompactCombo(std::format("##{}", sLabel).c_str(), "", ImGuiComboFlags_CustomPreview | ImGuiComboFlags_NoArrowButton | ImGuiComboFlags_HeightLarge))
		{
			if (!ActiveMap[uHash])
				sPreview = sInput = "";
			...
			std::deque<std::string> vValid = {};
			{
				...
				std::transform(sSearch.begin(), sSearch.end(), sSearch.begin(), [](unsigned char c) { return char(std::tolower(c)); });
				for (auto& sEntry : vEntries)
				{
					if (FNV1A::Hash32(sEntry) == FNV1A::Hash32Const("##Divider"))
					{
					...
					std::transform(sEntryLower.begin(), sEntryLower.end(), sEntryLower.begin(), [](unsigned char c) { return char(std::tolower(c)); });
			...
			if (!vValid.empty())
			{
				...
				for (int i = 0; i < vValid.size(); i++)
				{
					...
					ImVec2 vOriginalPos3 = GetCursorPos();
					if (FDropdownSelectable(std::format("##{}{}", sEntry, i).c_str(), nullptr, 0, bActive))
						*pVar = sEntry, bReturn = true;
		...
		if (BeginComboPreview())
		{
			...
			if (bTitle)
			{
				...
				SetCursorPos({ vOriginalPos2.x + H::Draw.Scale(6), valueY });
				TextUnformatted(TruncateText(ActiveMap[uHash] ? sPreview : *pVar, vSize.x - H::Draw.Scale(vEntries.empty() ? 12 : 30)).c_str());
		...
		SetCursorPos(vOriginalPos);
		AddRowSize(vOriginalPos, vSize + ImVec2(0, H::Draw.Scale(3)));
		DebugDummy({ vSize.x, GetRowSize(vSize.y + H::Draw.Scale(3)) });
	...
	inline bool FColorPicker(const char* sLabel, Color_t* pColor, int iFlags = FColorPickerEnum::None, ImVec2 vOffset = {}, ImVec2 vSize = { H::Draw.Scale(12), H::Draw.Scale(12) }, ImVec2 vIconOffset = {}, bool* pHovered = nullptr)
	{
		...
		if (iFlags & AttachedColorPicker)
		{
			iFlags |= FColorPickerEnum::RetainPosition;
		}
		else if (iFlags & (FColorPickerEnum::Left | FColorPickerEnum::Right))
		{
		...
		if (iFlags & AttachedColorPicker)
		{
			// The owning dropdown already reserved and positioned this swatch.
		}
		else if (iFlags & (FColorPickerEnum::Left | FColorPickerEnum::Full))
	...
	inline bool FMDropdown(const char* sLabel, std::vector<std::pair<std::string, Color_t>>* pVar, int iFlags = FDropdownEnum::None, int iSizeOffset = 0, bool* pHovered = nullptr)
	{
		...
		if (FBeginCompactCombo(std::format("##{}", sLabel).c_str(), "", ImGuiComboFlags_CustomPreview | ImGuiComboFlags_NoArrowButton | ImGuiComboFlags_HeightLarge))
		{
			...
			// material stuff (only gathered while the dropdown is open)
			std::vector<const Material_t*> vMaterials;
			...
			for (auto pMaterial : vMaterials)
				vEntries.push_back(pMaterial->m_sName.c_str());
			...
			for (int i = 0; i < vEntries.size(); i++)
			{
				...
				if (FDropdownSelectable(std::format("##{}{}", sEntry, i).c_str(), nullptr, 0, bFlagActive, ImGuiSelectableFlags_DontClosePopups))
				{
					...
					// the vector was modified, so the cached iterators are no longer valid
					mIts.clear();
					for (auto it2 = pVar->begin(); it2 != pVar->end(); it2++)
						mIts[it2->first] = it2;
				}
				...
				TextColored(bFlagActive ? F::Render.Active : F::Render.Inactive, (bHovered ? sEntry : TruncateText(sEntry, vSize.x - H::Draw.Scale(bFlagActive ? 81 : 57))).c_str());
				SameLine(); DebugDummy({ H::Draw.Scale(!GetCurrentWindow()->ScrollbarY ? 16 : 9), 0 });
	...
	inline bool bPushedDisabled = false, bPushedTransparent = false;
	...
	{ \
		...
		{	/*probably a better way to do this*/ \
			...
			{ \
				...
				std::transform(sBind.begin(), sBind.end(), sBind.begin(), [](unsigned char c) { return char(std::tolower(c)); }); \
	...
	// Dedicated left-hand color grid; no hard-coded offsets into neighboring controls.
	inline bool FColoredDropdown(ConfigVar<int>& setting, std::initializer_list<ConfigVar<Color_t>*> colors, int flags = 0)
	{
		const int count = int(colors.size());
		const int columns = std::max(1, (count + 1) / 2);
		const int rows = count > 1 ? 2 : 1;
		const float cell = H::Draw.Scale(12), gap = H::Draw.Scale(2);
		AttachedDropdownWidth = columns * cell + (columns - 1) * gap + H::Draw.Scale(4);
		const bool changed = FDropdown(setting, flags);
		const ImVec2 next = GetCursorPos();
		// Swatches are overlays within the dropdown row. SameLine reads the
		// previous item baseline, not just the cursor restored below.
		auto& layout = GetCurrentWindow()->DC;
		const ImVec2 previousPosition = layout.CursorPosPrevLine;
		const ImVec2 previousSize = layout.PrevLineSize;
		const ImVec2 currentSize = layout.CurrLineSize;
		const float previousBaseline = layout.PrevLineTextBaseOffset;
		const float currentBaseline = layout.CurrLineTextBaseOffset;
		const bool sameLine = layout.IsSameLine;
		const bool setPosition = layout.IsSetPos;
		const ImVec2 origin = AttachedDropdownOrigin;
		const float top = (AttachedDropdownHeight - (rows * cell + (rows - 1) * gap)) * 0.5f;
		int index = 0;
		for (auto* color : colors)
		{
			SetCursorPos(origin + ImVec2((index / rows) * (cell + gap), top + (index % rows) * (cell + gap)));
			FColorPicker(*color, AttachedColorPicker | FColorPickerEnum::Tooltip, {}, { cell, cell });
			++index;
		}
		SetCursorPos(next);
		layout.CursorPosPrevLine = previousPosition;
		layout.PrevLineSize = previousSize;
		layout.CurrLineSize = currentSize;
		layout.PrevLineTextBaseOffset = previousBaseline;
		layout.CurrLineTextBaseOffset = currentBaseline;
		layout.IsSameLine = sameLine;
		layout.IsSetPos = setPosition;
		return changed;
	}
