...
static inline CTFPlayer* GetPlayerFromUserID(int iUserID)
{
	int iIndex = I::EngineClient->GetPlayerForUserID(iUserID);
	if (iIndex <= 0) // not found (0 is the world)
		return nullptr;
	...
	return pPlayer->IsPlayer() ? pPlayer : nullptr;
}
...
void CResolver::CreateMove()
{
	...
	{
		if (auto pTarget = GetPlayerFromUserID(m_iWaitingForTarget))
		{
...
void CResolver::SetYaw(int iUserID, float flValue, bool bAuto)
{
	...
	F::Backtrack.ResolverUpdate(GetPlayerFromUserID(iUserID));
}
...
void CResolver::SetPitch(int iUserID, float flValue, bool bInverse, bool bAuto)
{
	...
	{
		auto pPlayer = GetPlayerFromUserID(iUserID);
