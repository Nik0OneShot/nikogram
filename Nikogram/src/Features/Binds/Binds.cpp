...
void CBinds::RemoveBind(int iBind, bool bForce)
{
	...
	{
		if (std::ranges::find(vErases, iIndex) != vErases.end())
			return; // already visited (guards against parent cycles)
...
{
	Bind_t tBind;
	for (size_t i = 0; i <= m_vBinds.size() && GetBind(iBind, &tBind); i++) // bounded in case of a parent cycle
	{
		if (!tBind.m_bEnabled)
			return false;
