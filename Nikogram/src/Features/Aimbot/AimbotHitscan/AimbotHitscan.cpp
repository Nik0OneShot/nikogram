...
void CAimbotHitscan::GetHitboxPoints(std::vector<Vec3>& vPoints, CBaseEntity* pTarget, CTFWeaponBase* pWeapon, const Vec3& vMins, const Vec3& vMaxs, int nHitbox)
{	// output vector is reused by the caller to avoid allocating per hitbox
	vPoints = { Vec3() };
...
bool CAimbotHitscan::BuildServerEstimate(CBaseEntity* pTarget, const std::vector<TickRecord*>& records)
{
	if (!Vars::Aimbot::Hitscan::Extrapolation.Value || records.size() < 2 || pTarget->IsDormant())
		return false;
	const auto& latest = *records[0];
	const auto& previous = *records[1];
	if (latest.m_bInvalid || previous.m_bInvalid)
		return false;
	const float scale = ServerEstimate::SampleScale(TICKS_TO_TIME(F::Backtrack.m_iTickCount),
		latest.m_flSimTime, previous.m_flSimTime, Vars::Aimbot::Hitscan::ExtrapolationRange.Value);
	if (scale <= 0.f)
		return false;
	const auto finite = [](const Vec3& v) { return std::isfinite(v.x) && std::isfinite(v.y) && std::isfinite(v.z); };
	const Vec3 sample = latest.m_vOrigin - previous.m_vOrigin;
	Vec3 offset = sample * scale;
	// Reject discontinuities and implausible displacement rather than amplifying them.
	if (!finite(latest.m_vOrigin) || !finite(sample) || !finite(offset)
	...
	CGameTrace trace = {};
	SDK::TraceHull(latest.m_vOrigin, latest.m_vOrigin + offset, mins, maxs, MASK_PLAYERSOLID, &filter, &trace);
	if (trace.startsolid || trace.allsolid || !std::isfinite(trace.fraction))
		return false;
	offset *= std::clamp(trace.fraction, 0.f, 1.f);
	if (offset.LengthSqr() < 0.0001f)
		return false;
	m_tServerEstimate = latest;
	m_vEstimateOffset = offset;
	m_flEstimateSeconds = scale * (latest.m_flSimTime - previous.m_flSimTime) * std::clamp(trace.fraction, 0.f, 1.f);
	m_tServerEstimate.m_vOrigin += offset;
	for (auto& bone : m_tServerEstimate.m_aBones)
	{
		bone[0][3] += offset.x;
		bone[1][3] += offset.y;
		bone[2][3] += offset.z;
	}
	// Keep the real sample timestamp; Run must never use this as a backtrack record.
	return true;
}
...
int CAimbotHitscan::CanHit(Target_t& tTarget, CTFPlayer* pLocal, CTFWeaponBase* pWeapon)
{
	...
	bool bServerEstimate = false;

	std::vector<TickRecord*> vRecords = {};
	if (F::Backtrack.GetRecords(tTarget.m_pEntity, vRecords) && !bWrangler)
	{
		bServerEstimate = bPlayer && pWeapon->GetWeaponID() != TF_WEAPON_MEDIGUN
			&& BuildServerEstimate(tTarget.m_pEntity, vRecords);
		vRecords = bServerEstimate ? std::vector<TickRecord*>{ &m_tServerEstimate }
			: F::Backtrack.GetValidRecords(vRecords, pLocal);
		if (vRecords.empty())
			return false;
	}
	else
		...
		for (auto& tHitbox : vHitboxes)
		{
			...
			for (auto& vPoint : vPoints)
			{
				...
				if (m_vEyePos.DistToSqr(vOrigin) > flMaxRangeSqr)
					continue;
				// The old entity pose must not hide an obstacle in front of the estimate.
				if (bServerEstimate && !SDK::VisPosWorld(pLocal, nullptr, m_vEyePos, vOrigin))
					continue;
					...
					&& (!pHullTransform || Math::RayToOBB(m_vEyePos, vForward, vHullMins, vHullMaxs, *pHullTransform)))
				{
					...
					if (bServerEstimate && Vars::Aimbot::Hitscan::ExtrapolationGuide.Value && !F::Aimbot.m_bRunningSecondary)
						G::ExtrapolationGuide = { vOrigin - m_vEstimateOffset, vOrigin, m_flEstimateSeconds, I::GlobalVars->tickcount,
							tTarget.m_pEntity->As<IHandleEntity>()->GetRefEHandle(), tHitbox.m_pBox ? tHitbox.m_iHitbox : -1 };
					if (tHitbox.m_pBox)
					{
						tTarget.m_bBacktrack = !bServerEstimate;
...
bool CAimbotHitscan::BuildOccludedGuide(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CBaseEntity* pTarget)
{
	// Display-only fallback: an obstructed shot must not suppress the ESP estimate.
	// Never publish this point as an aim target or cooldown crosshair/viewmodel point.
	if (!pTarget->IsPlayer() || pWeapon->GetWeaponID() == TF_WEAPON_MEDIGUN
		|| pWeapon->GetWeaponID() == TF_WEAPON_LASER_POINTER)
		return false;
	std::vector<TickRecord*> records;
	if (!F::Backtrack.GetRecords(pTarget, records) || !BuildServerEstimate(pTarget, records))
		return false;
	for (const auto& hitbox : GetHitboxes(pLocal, pWeapon, pTarget))
	{
		const auto* box = hitbox.m_pBox;
		if (!box || box->bone < 0 || box->bone >= MAXSTUDIOBONES) continue;
		Vec3 endpoint;
		Math::VectorTransform((box->bbmin + box->bbmax) * 0.5f, m_tServerEstimate.m_aBones[box->bone], endpoint);
		const Vec3 eye = pLocal->GetShootPos();
		if (eye.DistToSqr(endpoint) > powf(pWeapon->GetRange(), 2.f)
			|| !F::AimbotGlobal.ShouldAimAtAngle(Math::CalcAngle(eye, endpoint)))
			continue;
		G::ExtrapolationGuide = { endpoint - m_vEstimateOffset, endpoint, m_flEstimateSeconds, I::GlobalVars->tickcount,
			pTarget->As<IHandleEntity>()->GetRefEHandle(), hitbox.m_iHitbox };
		return true;
	}
	return false;
}
...
void CAimbotHitscan::Run(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CUserCmd* pCmd)
{
	...
	// Prefer the actual hittable solution; otherwise show the best eligible estimate
	// even through a wall. This pass does not alter CanHit or any shooting checks.
	if (guide && !G::ExtrapolationGuide.tick)
		for (auto& target : targets)
			if (BuildOccludedGuide(pLocal, pWeapon, target.m_pEntity)) break;
}

void CAimbotHitscan::RunMain(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CUserCmd* pCmd)
{
