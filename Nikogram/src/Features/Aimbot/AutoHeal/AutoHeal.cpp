...
static inline int GetShotsWithinTime(int iWeaponID, float flFireRate, float flTime)
{
	...
	return std::max(int(1 + (iTicks - iDelay) / std::max(std::ceilf(flFireRate / TICK_INTERVAL), 1.f)), 1);
}
...
void CAutoHeal::GetDangers(CTFPlayer* pTarget, bool bVaccinator, float& flBulletOut, float& flBlastOut, float& flFireOut)
{
	...
	for (auto pEntity : H::Entities.GetGroup(EntityEnum::PlayerEnemy))
	{
		...
		case TF_WEAPON_SNIPERRIFLE_CLASSIC:
		{
			...
			{
				float flChargeRate = SDK::AttribHookValue(1.f, "mult_sniper_charge_per_sec", pWeapon); // rate multiplier, faster charge = shorter time
				float flChargeTime = std::max(flChargeRate > 0.f ? 3.f / flChargeRate : 3.f, 1.5f);
