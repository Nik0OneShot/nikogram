...
void CWorld::UncacheBoxBrushes()
{
	...
	for (auto it = m_mFaceCache.begin(); it != m_mFaceCache.end();)
	{
		if (!it->second.empty() && it->second.front().m_iType == FaceTypeEnum::BoxBrush)
			it = m_mFaceCache.erase(it);
		else
			++it;
	}
...
void CWorld::UncachePlaneBrushes()
{
	...
	for (auto it = m_mFaceCache.begin(); it != m_mFaceCache.end();)
	{
		if (!it->second.empty() && it->second.front().m_iType == FaceTypeEnum::PlaneBrush)
			it = m_mFaceCache.erase(it);
		else
			++it;
	}
...
void CWorld::UncacheProps()
{
	...
	for (auto it = m_mFaceCache.begin(); it != m_mFaceCache.end();)
	{
		if (!it->second.empty() && it->second.front().m_iType == FaceTypeEnum::Prop)
			it = m_mFaceCache.erase(it);
		else
			++it;
	}
...
void CWorld::UncacheEntities()
{
	...
	for (auto it = m_mFaceCache.begin(); it != m_mFaceCache.end();)
	{
		if (!it->second.empty() && it->second.front().m_iType == FaceTypeEnum::Entity)
			it = m_mFaceCache.erase(it);
		else
			++it;
	}
