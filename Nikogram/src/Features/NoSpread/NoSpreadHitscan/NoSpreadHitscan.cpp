...
bool CNoSpreadHitscan::ParsePlayerPerf(const std::string& sMsg)
{
	...
	static const std::regex tPerfRegex(R"((\d+\.\d+)\s\d+\s\d+\s\d+\.\d+\s\d+\.\d+\svel\s\d+\.\d+)");
	std::smatch tMatch; std::regex_match(sMsg, tMatch, tPerfRegex);
	...
	static const std::regex tPerfRegex2(R"(\d+\.\d+\s\d+\s\d+)");
	return std::regex_match(sMsg, tPerfRegex2);
}
