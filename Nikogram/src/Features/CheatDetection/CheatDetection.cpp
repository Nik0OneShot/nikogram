...
void CCheatDetection::Run()
{
	...
	{	// prune data of players that are no longer present
	...
	for (auto& pEntity : vPlayers)
	{
		...
		if (auto it = mData.find(pPlayer); it != mData.end() && it->second.m_uAccountID && it->second.m_uAccountID != uint32_t(pResource->m_iAccountID(iIndex)))
			mData.erase(it); // entity was reused by a different player
