...
{
	if (m_pMatGlowColor && m_pMatBlurX && m_pMatBlurY && m_pMatHaloAddToScreen && m_pRenderBuffer1 && m_pRenderBuffer2)
		return true;

	// don't reload every frame from inside render hooks, just skip glow while the materials are missing
	static double dLastReload = 0.0;
	const double dTime = SDK::PlatFloatTime();
	if (dTime - dLastReload >= 5.0)
	{
		dLastReload = dTime;
...
void CGlow::RenderBacktrack(const DrawModelState_t& pState, const ModelRenderInfo_t& pInfo)
{
	...
	auto fDrawModel = [&](Vec3& vOrigin, const DrawModelState_t& pState, const ModelRenderInfo_t& pInfo, matrix3x4* pBoneToWorld)
	{
	...
	if (!bDrawLast && !bDrawFirst)
	{
		...
		{
			if (Math::RemapVal(pEntity->GetAbsOrigin().DistToSqr(pRecord->m_vOrigin), 1.f, 576.f, 0.f, 1.f))
				fDrawModel(pRecord->m_vOrigin, pState, pInfo, pRecord->m_aBones);
		}
	}
	else
	{
