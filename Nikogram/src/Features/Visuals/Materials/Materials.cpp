...
void CMaterials::LoadMaterials()
{
	...
	for (auto& tMaterial : m_mMaterials | std::views::values)
	{
		...
		{
			kv->DeleteThis(); // CreateMaterial never took ownership
			continue;
		}
...
void CMaterials::EditMaterial(const char* sName, const char* sVMT)
{
	...
	if (m_mMaterials.contains(uHash) && !m_mMaterials[uHash].m_bLocked)
	{
		...
		Remove(tMaterial.m_pMaterial);
		tMaterial.m_pMaterial = nullptr; // Remove() only clears its own copy of the pointer
		...
		{
			// previously this returned with m_bLoaded still false, which switched off chams/glow until a reload
			kv->DeleteThis();
			m_bLoaded = true;
			SDK::Output("Nikogram", std::format("Material {} has invalid VMT", sName).c_str(), ERROR_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU | OUTPUT_DEBUG, ICON_MD_CANCEL);
			return;
		}
