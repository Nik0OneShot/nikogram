...
void CMisc::WeaponSway()
{
	...
	static bool bSwaySet = false;
	static float flOldInterp = 0.f, flOldScale = 0.f;

	bool bSway = Vars::Visuals::Viewmodel::SwayInterp.Value || Vars::Visuals::Viewmodel::SwayScale.Value;
	if (bSway)
	{
		if (!bSwaySet)
		{	// remember the user's values so they can be restored
			flOldInterp = cl_wpn_sway_interp->GetFloat();
			flOldScale = cl_wpn_sway_scale->GetFloat();
			bSwaySet = true;
		}

		if (cl_wpn_sway_interp->GetFloat() != Vars::Visuals::Viewmodel::SwayInterp.Value)
			cl_wpn_sway_interp->SetValue(Vars::Visuals::Viewmodel::SwayInterp.Value);
		if (cl_wpn_sway_scale->GetFloat() != Vars::Visuals::Viewmodel::SwayScale.Value)
			cl_wpn_sway_scale->SetValue(Vars::Visuals::Viewmodel::SwayScale.Value);
	}
	else if (bSwaySet)
	{
		cl_wpn_sway_interp->SetValue(flOldInterp);
		cl_wpn_sway_scale->SetValue(flOldScale);
		bSwaySet = false;
	}
}
