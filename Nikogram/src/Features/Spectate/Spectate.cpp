...
void CSpectate::NetUpdateEnd(CTFPlayer* pLocal)
{
	...
	if (HasTarget())
	{
		int iIndex = I::EngineClient->GetPlayerForUserID(m_iTarget);
		if (iIndex > 0)
		{
			pEntity = I::ClientEntityList->GetClientEntity(iIndex)->As<CTFPlayer>();
			if (pEntity && !pEntity->IsPlayer())
				pEntity = nullptr;
		}
		if (iIndex <= 0 || pEntity == pLocal) // target left or is local
			m_iTarget = m_iIntendedTarget = -1, pEntity = nullptr;
	}
	if (!HasTarget())
	{
